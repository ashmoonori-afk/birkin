...
version: 1.0.0
author: arceus77-7, enhanced by Birkin
license: MIT
metadata:
  birkin:
...
- `tmux` available for stable authenticated sessions during Birkin terminal calls (desktop app flow only)
...
### Service Account (recommended for Birkin)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.birkin/.env` (the skill will prompt for this on first load).
...
## Birkin Execution Pattern (desktop app flow)

Birkin terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/birkin-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/birkin-op.sock"
