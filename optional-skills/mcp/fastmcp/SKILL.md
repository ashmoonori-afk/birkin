...
- smoke-test a server with the FastMCP CLI before wiring it into Birkin or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Birkin. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.birkin/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.birkin/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Birkin integration, either:

- configure the server in `~/.birkin/config.yaml` using the `native-mcp` skill, or
...
### Birkin cannot see the deployed server

The server-building part may be correct while the Birkin config is not. Load the `native-mcp` skill and configure the server in `~/.birkin/config.yaml`, then restart Birkin.
