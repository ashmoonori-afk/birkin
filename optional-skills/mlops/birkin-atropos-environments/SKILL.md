---
name: birkin-atropos-environments
description: Build, test, and debug Birkin RL environments for Atropos training. Covers the BirkinAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or fixing RL environments in the birkin-agent repo.
version: 1.1.0
author: Birkin
license: MIT
metadata:
...
# Birkin Atropos Environments

Guide for building RL environments in the birkin-agent repo that integrate with the Atropos training framework.

## Architecture Overview

```
Atropos BaseEnv (atroposlib/envs/base.py)
    └── BirkinAgentBaseEnv (environments/birkin_base_env.py)
...
Birkin environments are special because they run a **multi-turn agent loop with tool calling** — not just single-turn completions. The base env handles the loop; you implement the task and scoring.

## File Locations

| File | Purpose |
|------|---------|
| `environments/birkin_base_env.py` | Base class with agent loop + tool resolution |
| `environments/agent_loop.py` | `BirkinAgentLoop` + `AgentResult` dataclass |
| `environments/tool_context.py` | `ToolContext` for reward verification |
| `environments/tool_call_parsers.py` | Phase 2 tool call parsers (birkin, mistral, etc.) |
...
The whole point of birkin-agent environments is agentic evaluation:

```python
async def evaluate(self, *args, **kwargs) -> None:
    import time, uuid
    from environments.agent_loop import BirkinAgentLoop
...
2. **evaluate() must use BirkinAgentLoop, not chat_completion** — Single-turn chat_completion has no tools. The whole point of birkin-agent benchmarks is agentic evaluation with tool use.
