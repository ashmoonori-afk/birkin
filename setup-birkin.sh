...
# 5. Symlinks the 'birkin' CLI command into a user-facing bin dir
...
# ============================================================================
# PATH setup — symlink birkin into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up birkin command..."

BIRKIN_BIN="$SCRIPT_DIR/venv/bin/birkin"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$BIRKIN_BIN" "$COMMAND_LINK_DIR/birkin"
echo -e "${GREEN}✓${NC} Symlinked birkin → $COMMAND_LINK_DISPLAY_DIR/birkin"
...
# ============================================================================
# Seed bundled skills into ~/.birkin/skills/
# ============================================================================

BIRKIN_SKILLS_DIR="${BIRKIN_HOME:-$HOME/.birkin}/skills"
mkdir -p "$BIRKIN_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.birkin/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$BIRKIN_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  birkin status        # Check configuration"
if is_termux; then
    echo "  birkin gateway       # Run gateway in foreground"
else
    echo "  birkin gateway install # Install gateway service (messaging + cron)"
fi
echo "  birkin cron list     # View scheduled jobs"
echo "  birkin doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m birkin_cli.main setup
fi
