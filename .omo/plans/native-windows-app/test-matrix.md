...
- jailed imports, Python spreadsheet comparison, and a visible read-only Diff;
  generic UI approval controls answer only canonical Python approval records,
  while comparison-report save remains unavailable until a durable
  multi-artifact Office job exists;
- deterministic fast regressions use the production-composed real WPF window,
  ...
  phase-exit gate is protected and exercises the same read-only boundary; it
  does not claim structural report output;
