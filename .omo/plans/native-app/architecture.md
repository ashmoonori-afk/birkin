...
Production packaging refuses a dirty tree and embeds signed architecture-specific Python helpers built with the app, plus checksum-pinned arm64/x86_64 Chromium headless-shell and FFmpeg trees. The app-signature-sealed manifest records that clean revision and exact package version. Swift verifies the selected helper executable and requires both the manifest package version and the bridge `ready.server_version` to equal its generated product version. Frozen Python independently verifies the matching browser tree and its package version before setting `PLAYWRIGHT_BROWSERS_PATH`; there is no host Python, repository, virtualenv, or Playwright-cache fallback. The helper remains the authority and uses the same modules as the CLI. A developer bridge override does not bypass the exact product-version handshake.

`BIRKIN_NATIVE_JOURNEY=1` is a disabled-by-default release-QA seam in the packaged app. It drives the same control closures as the SwiftUI shell, never a test transport or direct wire client, and remains subject to Python authentication, approvals, consent, and policy. Package acceptance requires a real existing-account provider probe and a separate provider-backed chat completion with exact success markers before the full Browser, Office, Memory, import, terminal, and reconnect journey is accepted.

The public V1 distribution target is Developer ID signing with hardened runtime, no App Sandbox, minimal entitlements, notarization, stapling, and Gatekeeper assessment. The repository packaging script uses that signing mode only when a Developer ID identity is available. Without one it produces an inside-out ad-hoc-signed development artifact with no hardened-runtime option; it does not notarize that artifact. PTY, local socket, Accessibility, and Screen Recording integration remain the documented reason App Sandbox is disabled.
