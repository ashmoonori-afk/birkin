"""
Birkin — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m birkin_cli.main web          # Start on http://127.0.0.1:9119
    python -m birkin_cli.main web --port 8080
"""
...
try:
    ...
    raise SystemExit(
        ...
        "Run 'birkin web' to auto-install, or: pip install birkin-agent[web]"
    )

WEB_DIST = Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Birkin", version=__version__)
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Birkin supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# the canonical ``birkin auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Birkin resolves Anthropic creds in this order at runtime:
    1. ``~/.birkin/.anthropic_oauth.json`` — Birkin-managed PKCE flow
    ...
    try:
        ...
        read_birkin_oauth_credentials = None  # type: ignore
        _BIRKIN_OAUTH_FILE = None  # type: ignore

    birkin_creds = None
    if read_birkin_oauth_credentials:
        try:
            birkin_creds = read_birkin_oauth_credentials()
        except Exception:
            birkin_creds = None
    if birkin_creds and birkin_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "birkin_pkce",
            "source_label": f"Birkin PKCE ({_BIRKIN_OAUTH_FILE})",
            "token_preview": _truncate_token(birkin_creds.get("accessToken")),
            "expires_at": birkin_creds.get("expiresAt"),
            "has_refresh_token": bool(birkin_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Birkin even
    when they also have a separate Birkin-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "birkin auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "birkin auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "birkin auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "birkin auth add qwen-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from birkin_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("birkin_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Birkin-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in ("anthropic", "claude-code"):
        try:
            from agent.anthropic_adapter import _BIRKIN_OAUTH_FILE
            if _BIRKIN_OAUTH_FILE.exists():
                _BIRKIN_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from birkin_cli.auth import clear_provider_auth
...
#          → persists to ~/.birkin/.anthropic_oauth.json AND credential pool
...
# Guarded so birkin web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Birkin file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``birkin auth add anthropic``.
    """
    from agent.anthropic_adapter import _BIRKIN_OAUTH_FILE
    payload = {
    ...
    _BIRKIN_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _BIRKIN_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "birkin-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    from birkin_cli import auth as hauth
    if provider_id == "nous":
        from birkin_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("BIRKIN_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from birkin_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
...
async def get_logs(
    ...
    from birkin_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_birkin_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from birkin_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def get_skills():
    ...
    from birkin_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from birkin_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from birkin_cli.tools_config import (
