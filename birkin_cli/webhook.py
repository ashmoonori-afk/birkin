"""birkin webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.birkin/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _birkin_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: birkin gateway run
"""
...
def webhook_command(args):
    """Entry point for 'birkin webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: birkin webhook {subscribe|list|remove|test}")
        print("Run 'birkin webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (birkin gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: birkin webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from birkin webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (birkin gateway run)")
