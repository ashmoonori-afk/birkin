#!/usr/bin/env python3
"""
Birkin CLI - Main entry point.

Usage:
    birkin                     # Interactive chat (default)
    birkin chat                # Interactive chat
    birkin gateway             # Run gateway in foreground
    birkin gateway start       # Start gateway as service
    birkin gateway stop        # Stop gateway service
    birkin gateway status      # Show gateway status
    birkin gateway install     # Install gateway service
    birkin gateway uninstall   # Uninstall gateway service
    birkin setup               # Interactive setup wizard
    birkin logout              # Clear stored authentication
    birkin status              # Show status of all components
    birkin cron                # Manage cron jobs
    birkin cron list           # List cron jobs
    birkin cron status         # Check if cron scheduler is running
    birkin doctor              # Check configuration and dependencies
    birkin honcho setup                    # Configure Honcho AI memory integration
    birkin honcho status                   # Show Honcho config and connection status
    birkin honcho sessions                 # List directory → session name mappings
    birkin honcho map <name>               # Map current directory to a session name
    birkin honcho peer                     # Show peer names and dialectic settings
    birkin honcho peer --user NAME         # Set user peer name
    birkin honcho peer --ai NAME           # Set AI peer name
    birkin honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    birkin honcho mode                     # Show current memory mode
    birkin honcho mode [hybrid|honcho|local]  # Set memory mode
    birkin honcho tokens                   # Show token budget settings
    birkin honcho tokens --context N       # Set session.context() token cap
    birkin honcho tokens --dialectic N     # Set dialectic result char cap
    birkin honcho identity                 # Show AI peer identity representation
    birkin honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    birkin honcho migrate                  # Step-by-step migration guide: OpenClaw native → Birkin + Honcho
    birkin version             Show version
    birkin update              Update to latest version
    birkin uninstall           Uninstall Birkin
    birkin acp                 Run as an ACP server for editor integration
    birkin sessions browse     Interactive session picker with search

    birkin claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (birkin tools, birkin setup, birkin model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'birkin {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any birkin module import.
#
# Many modules cache BIRKIN_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("BIRKIN_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.birkin/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set BIRKIN_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check active_profile in the birkin root
    if profile_name is None:
        try:
            from birkin_constants import get_default_birkin_root
            active_path = get_default_birkin_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set BIRKIN_HOME
    if profile_name is not None:
        try:
            from birkin_cli.profiles import resolve_profile_env
            birkin_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent birkin from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["BIRKIN_HOME"] = birkin_home
...
# Load .env from ~/.birkin/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from birkin_cli.config import get_birkin_home
from birkin_cli.env_loader import load_birkin_dotenv
load_birkin_dotenv(project_env=PROJECT_ROOT / '.env')

# Initialize centralized file logging early — all `birkin` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from birkin_logging import setup_logging as _setup_logging
...
try:
    from birkin_cli.config import load_config as _load_config_early
    from birkin_constants import apply_ipv4_preference as _apply_ipv4
...
from birkin_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from birkin_cli.config import get_env_path, get_birkin_home, load_config
    from birkin_cli.auth import get_auth_status

    # Determine whether Birkin itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from birkin_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_birkin_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from birkin_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_birkin_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Birkin has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Birkin to use their tokens.
    if _has_birkin_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, birkin_bin
        cli_args: the original CLI arguments (everything after 'birkin')
    """
    import shutil
    ...
    birkin_bin = container_info["birkin_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f'  }}];\n'
                    f"\n"
                    f"Or run: sudo birkin {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo birkin {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'birkin sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Birkin isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  birkin setup")
        print()

        from birkin_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'birkin setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from birkin_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["BIRKIN_YOLO_MODE"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["BIRKIN_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from birkin_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from birkin_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Birkin?")
        print()
    ...
    session_dir = get_birkin_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: birkin gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  birkin gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Birkin'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  birkin gateway")
        ...
        print()
        print("  Or install as a service: birkin gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'birkin whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from birkin_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``birkin model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from birkin_cli.auth import (
        resolve_provider, AuthError, format_auth_error,
    )
    from birkin_cli.config import get_compatible_custom_providers, load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("BIRKIN_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
    ...
    from birkin_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.birkin/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.birkin/.env if the active provider is not 'custom'.
    ...
    """
    from birkin_cli.config import get_env_value, save_env_value, load_config
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from birkin_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from birkin_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from birkin_cli.config import get_env_value, save_env_value
    ...
    from birkin_cli.models import model_ids, get_pricing_for_provider
    ...
    if selected:
        ...
        from birkin_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from birkin_cli.config import get_env_value, save_config, save_env_value
    from birkin_cli.nous_subscription import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from birkin_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from birkin_cli.codex_models import get_codex_model_ids
    import argparse
    ...
    _codex_token = None
    # Prefer credential pool (where `birkin auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from birkin_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from birkin_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from birkin_cli.auth import _save_model_choice, deactivate_provider
    from birkin_cli.config import get_env_value, load_config, save_config
    ...
    from birkin_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `birkin model` to set a model.")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from birkin_cli.curses_ui import flush_stdin
        flush_stdin()
        print()
...
from birkin_cli.models import _PROVIDER_MODELS
...
def _model_flow_copilot(config, current_model=""):
    ...
    from birkin_cli.config import save_env_value, load_config, save_config
    from birkin_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from birkin_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from birkin_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Birkin turns to `copilot --acp`.")
    print("  Birkin currently starts its own ACP subprocess for each request.")
    print("  Birkin uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set BIRKIN_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from birkin_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from birkin_cli.config import get_env_value, save_env_value, load_config, save_config
    from birkin_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from birkin_constants import display_birkin_home as _dhh_fn
            print(f"    Birkin will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from birkin_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'birkin model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Birkin CLI with a provider."""
    from birkin_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from birkin_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from birkin_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from birkin_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from birkin_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from birkin_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from birkin_cli.doctor import run_doctor
    run_doctor(args)


def cmd_dump(args):
    """Dump setup summary for support/debugging."""
    from birkin_cli.dump import run_dump
    run_dump(args)


def cmd_debug(args):
    """Debug tools (share report, etc.)."""
    from birkin_cli.debug import run_debug
    run_debug(args)


def cmd_config(args):
    """Configuration management."""
    from birkin_cli.config import config_command
    config_command(args)


def cmd_backup(args):
    """Back up Birkin home directory to a zip file."""
    if getattr(args, "quick", False):
        from birkin_cli.backup import run_quick_backup
        run_quick_backup(args)
    else:
        from birkin_cli.backup import run_backup
        run_backup(args)


def cmd_import(args):
    """Restore a Birkin backup from a zip file."""
    from birkin_cli.backup import run_import
    run_import(args)


def cmd_version(args):
    """Show version."""
    print(f"Birkin v{__version__} ({__release_date__})")
    ...
    try:
        from birkin_cli.banner import check_for_updates
        from birkin_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from birkin_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``birkin update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``birkin web``).
    ...
    if r1.returncode != 0:
        print(f"  {'✗' if fatal else '⚠'} Web UI npm install failed"
              + ("" if fatal else " (birkin web will not be available)"))
        if fatal:
...
def _update_via_zip(args):
    """Update Birkin by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/NousResearch/birkin-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="birkin-update-")
        zip_path = os.path.join(tmp_dir, f"birkin-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to birkin-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"birkin-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("birkin-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Birkin behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes birkin completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Birkin couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Birkin couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Birkin behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `birkin update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/birkin-agent.git",
    "git@github.com:NousResearch/birkin-agent.git",
    "https://github.com/NousResearch/birkin-agent",
    "git@github.com:NousResearch/birkin-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/birkin-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    from birkin_constants import get_birkin_home
    return (get_birkin_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from birkin_constants import get_birkin_home
        (get_birkin_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Birkin repository.")
        print("  This means you may miss updates from NousResearch/birkin-agent.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/NousResearch/birkin-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/NousResearch/birkin-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``birkin update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from birkin_constants import get_default_birkin_root
    default_home = get_default_birkin_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "birkin-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update Birkin to the latest version."""
    import shutil
    from birkin_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Birkin")
        return
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/NousResearch/birkin-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_birkin_home added to birkin_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload birkin_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_birkin_home() added since the last release.
        try:
            import importlib
            import birkin_constants as _hc
        ...
        try:
            from birkin_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'birkin config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: birkin config migrate")
            else:
                print()
                print("Skipped. Run 'birkin config migrate' later to configure.")
        else:
        ...
        # When running as ``birkin update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart birkin-gateway`` kills everything in the
        ...
        if gateway_mode:
            _exit_code_path = get_birkin_home() / ".update_exit_code"
            try:
        ...
        try:
            from birkin_cli.gateway import (
            ...
            # Discover all birkin-gateway* units (default + profiles)
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "birkin-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. birkin-gateway.service or birkin-gateway-coder.service
            ...
            if is_macos():
                try:
                    from birkin_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: birkin gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: birkin -p <profile> gateway run  for each profile)")
        ...
        print("  birkin model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``birkin -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from birkin_cli.profiles import (
    ...
    from birkin_constants import display_birkin_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `birkin profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_birkin_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → birkin -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.birkin)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  birkin profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     birkin -p {name} chat")
                else:
        ...
        name = args.profile_name
        from birkin_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from birkin_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from birkin_cli.profiles import rename_profile
        try:
        ...
        from birkin_cli.profiles import export_profile
        name = args.profile_name
        ...
        from birkin_cli.profiles import import_profile
        try:
...
def cmd_dashboard(args):
    ...
    try:
        ...
        print("Install them with:  pip install birkin-agent[web]")
        sys.exit(1)

    if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
        sys.exit(1)

    from birkin_cli.web_server import start_server
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args):
    ...
    from birkin_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def cmd_logs(args):
    """View and filter Birkin log files."""
    from birkin_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for birkin CLI."""
    parser = argparse.ArgumentParser(
        prog="birkin",
        description="Birkin - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    birkin                        Start interactive chat
    birkin chat -q "Hello"        Single query mode
    birkin -c                     Resume the most recent session
    birkin -c "my project"        Resume a session by name (latest in lineage)
    birkin --resume <session_id>  Resume a specific session by ID
    birkin setup                  Run setup wizard
    birkin logout                 Clear stored authentication
    birkin auth add <provider>    Add a pooled credential
    birkin auth list              List pooled credentials
    birkin auth remove <p> <t>    Remove pooled credential by index, id, or label
    birkin auth reset <provider>  Clear exhaustion status for a provider
    birkin model                  Select default model
    birkin config                 View configuration
    birkin config edit            Edit config in $EDITOR
    birkin config set model gpt-4 Set a config value
    birkin gateway                Run messaging gateway
    birkin -s birkin-agent-dev,github-auth
    birkin -w                     Start in isolated git worktree
    birkin gateway install        Install gateway background service
    birkin sessions list          List past sessions
    birkin sessions browse        Interactive session picker
    birkin sessions rename ID T   Rename/title a session
    birkin logs                   View agent.log (last 50 lines)
    birkin logs -f                Follow agent.log in real time
    birkin logs errors            View errors.log
    birkin logs --since 1h        Lines from the last hour
    birkin debug share             Upload debug report for support
    birkin update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Birkin"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: birkin-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Birkin with an interactive wizard. "
                    "Run a specific section: birkin setup model|tts|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Birkin CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: birkin-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Birkin components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Birkin setup"
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Birkin setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Birkin. Use 'birkin debug share' to "
                    "upload a debug report (system info + recent logs) to a paste "
                    "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    birkin debug share              Upload debug report and print URL
    birkin debug share --lines 500  Include more log lines
    birkin debug share --expire 30  Keep paste for 30 days
    birkin debug share --local      Print report locally (no upload)
""",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Birkin home directory to a zip file",
        description="Create a zip archive of your entire Birkin configuration, "
                    "skills, sessions, and data (excludes the birkin-agent codebase). "
                    "Use --quick for a fast snapshot of just critical state files."
    )
    backup_parser.add_argument(
        "-o", "--output",
        help="Output path for the zip file (default: ~/birkin-backup-<timestamp>.zip)"
    )
    backup_parser.add_argument(
        "-q", "--quick",
        action="store_true",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Birkin backup from a zip file",
        description="Extract a previously created Birkin backup into your "
                    "Birkin home directory, restoring configuration, skills, "
                    "sessions, and data"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Birkin configuration"
    )
    ...
    def cmd_pairing(args):
        from birkin_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from birkin_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from birkin_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/birkin-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from birkin_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from birkin_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'birkin tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # birkin tools list [--platform cli]
    ...
    # birkin tools disable <name...> [--platform cli]
    ...
    # birkin tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from birkin_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from birkin_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Birkin as an MCP server",
        description=(
            "Manage MCP server connections and run Birkin as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'birkin mcp add' to connect to a new server, or\n"
            "'birkin mcp serve' to expose Birkin conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Birkin as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from birkin_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch birkin --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            birkin_bin = shutil.which("birkin")
            if birkin_bin:
                os.execvp(birkin_bin, ["birkin", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "birkin_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Birkin"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Birkin",
    ...
    def cmd_claw(args):
        from birkin_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Birkin to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Birkin",
        description="Remove Birkin from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Birkin as an ACP (Agent Client Protocol) server",
        description="Start Birkin in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Birkin as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Birkin instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Birkin web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Birkin log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    birkin logs                    Show last 50 lines of agent.log
    birkin logs -f                 Follow agent.log in real time
    birkin logs errors             Show last 50 lines of errors.log
    birkin logs gateway -n 100     Show last 100 lines of gateway.log
    birkin logs --level WARNING    Only show WARNING and above
    birkin logs --session abc123   Filter by session ID
    birkin logs --component tools  Only show tool-related lines
    birkin logs --since 1h         Lines from the last hour
    birkin logs --since 30m -f     Follow, starting from 30 min ago
    birkin logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``birkin -c Pokemon Agent Dev`` → ``birkin -c 'Pokemon Agent Dev'``
    ...
    from birkin_cli.config import get_container_exec_info
