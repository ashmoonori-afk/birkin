"""
Configuration management for Birkin.

Config files are stored in ~/.birkin/ for easy access:
- ~/.birkin/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.birkin/.env         - API keys and secrets

This module provides:
- birkin config          - Show current configuration
- birkin config edit     - Open config in editor
- birkin config set      - Set a specific value
- birkin config wizard   - Re-run setup wizard
"""
...
from birkin_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("BIRKIN_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_birkin_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Birkin is running in package-manager-managed mode.

    Two signals: the BIRKIN_MANAGED env var (set by the systemd service),
    or a .managed marker file in BIRKIN_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade birkin-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "birkin update"


def format_managed_message(action: str = "modify this Birkin installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("BIRKIN_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Birkin installation is managed by NixOS "
            f"(BIRKIN_MANAGED={env_hint}).\n"
            "Edit services.birkin-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Birkin installation is managed by Homebrew "
            f"(BIRKIN_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade birkin-agent"
        )

    return (
        f"Cannot {action}: this Birkin installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Birkin."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from BIRKIN_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, birkin_bin
    or None if container mode is not active, we're already inside the
    container, or BIRKIN_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("BIRKIN_DEV") == "1":
        return None
    ...
    container_mode_file = get_birkin_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "birkin-agent")
    exec_user = info.get("exec_user", "birkin")
    birkin_bin = info.get("birkin_bin", "/data/current-package/bin/birkin")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "birkin_bin": birkin_bin,
    }
...
# Re-export from birkin_constants — canonical definition lives there.
from birkin_constants import get_birkin_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_birkin_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_birkin_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the birkin group can
    share state with the gateway service.

    The mode can be overridden via the BIRKIN_HOME_MODE environment variable
    (e.g. BIRKIN_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse BIRKIN_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("BIRKIN_HOME_MODE", "").strip()
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into BIRKIN_HOME if the user doesn't have one yet."""
...
def ensure_birkin_home():
    """Ensure ~/.birkin directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_birkin_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_birkin_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"BIRKIN_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when Birkin
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Birkin sends a stable profile-scoped userId to Camofox
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.birkin/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.birkin/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for birkin-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Birkin*" header.
    ...
    # Logging — controls file logging to ~/.birkin/logs/.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Birkin auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @birkin:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. BIRKIN_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'birkin-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    # BIRKIN_TOOL_PROGRESS and BIRKIN_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "BIRKIN_TOOL_PROGRESS": {
    ...
    "BIRKIN_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.birkin.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'birkin setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Birkin won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'birkin doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("BIRKIN_TOOL_PROGRESS")
            old_mode = get_env_value("BIRKIN_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from BIRKIN_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from BIRKIN_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("BIRKIN_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from BIRKIN_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: birkin config set <key> <value>")
    ...
    # metadata.birkin.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.birkin/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.birkin/config.yaml."""
    import copy
    ensure_birkin_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — birkin auth) — OpenAI Codex
#   nous         (OAuth — birkin auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.birkin/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.birkin/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Birkin env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.birkin/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.birkin/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.birkin/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.birkin/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Birkin — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Birkin vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.birkin/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Birkin Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  birkin config edit     # Edit config file", Colors.DIM))
    print(color("  birkin config set <key> <value>", Colors.DIM))
    print(color("  birkin setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: birkin config set <key> <value>")
            print()
            print("Examples:")
            print("  birkin config set model anthropic/claude-sonnet-4")
            print("  birkin config set terminal.backend docker")
            print("  birkin config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'birkin config migrate' to add them")
        ...
        print("  birkin config           Show current configuration")
        print("  birkin config edit      Open config in editor")
        print("  birkin config set <key> <value>   Set a config value")
        print("  birkin config check     Check for missing/outdated config")
        print("  birkin config migrate   Update config with new options")
        print("  birkin config path      Show config file path")
        print("  birkin config env-path  Show .env file path")
        sys.exit(1)
