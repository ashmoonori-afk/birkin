"""
Profile management for multiple isolated Birkin instances.

Each profile is a fully independent BIRKIN_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.birkin/profiles/<name>/`` by default.

The "default" profile is ``~/.birkin`` itself — backward compatible,
zero migration needed.

Usage::

    birkin profile create coder          # fresh profile + bundled skills
    birkin profile create coder --clone  # also copy config, .env, SOUL.md
    birkin profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    birkin -p coder chat                 # or via flag
    birkin profile use coder             # set as sticky default
    birkin profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See birkin_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.birkin) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "birkin-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "birkin", "default", "test", "tmp", "root", "sudo",
})

# Birkin subcommands that cannot be used as profile names/aliases
_BIRKIN_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the birkin root, NOT to the current BIRKIN_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where BIRKIN_HOME points outside
    ``~/.birkin``, profiles live under ``BIRKIN_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_birkin_home() / "profiles"


def _get_default_birkin_home() -> Path:
    """Return the default (pre-profile) BIRKIN_HOME path.

    In standard deployments this is ``~/.birkin``.
    In Docker/custom deployments where BIRKIN_HOME is outside ``~/.birkin``
    (e.g. ``/opt/data``), returns BIRKIN_HOME directly.
    """
    from birkin_constants import get_default_birkin_root
    return get_default_birkin_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_birkin_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.birkin
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its BIRKIN_HOME directory."""
    if name == "default":
        return _get_default_birkin_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, birkin subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _BIRKIN_SUBCOMMANDS:
        return f"'{name}' conflicts with a birkin subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_birkin_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.birkin)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches BIRKIN_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "BIRKIN_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.birkin).\n"
            "To remove everything, use: birkin uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set BIRKIN_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("BIRKIN_HOME")
    try:
        os.environ["BIRKIN_HOME"] = str(profile_dir)
        from birkin_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["BIRKIN_HOME"] = old_home
        elif "BIRKIN_HOME" in os.environ:
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.birkin/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: birkin profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from BIRKIN_HOME.

    Returns ``"default"`` if BIRKIN_HOME is not set or points to ``~/.birkin``.
    Returns the profile name if BIRKIN_HOME points into ``~/.birkin/profiles/<name>``.
    Returns ``"custom"`` if BIRKIN_HOME is set to an unrecognized path.
    """
    from birkin_constants import get_birkin_home
    birkin_home = get_birkin_home()
    resolved = birkin_home.resolve()

    default_resolved = _get_default_birkin_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.birkin itself — its parent is ~/ and its
        # directory name is ".birkin", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: birkin profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.birkin itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.birkin). "
            "Specify a different name: birkin profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for birkin profile names."""
    return '''# Birkin profile completion
# Add to ~/.bashrc: eval "$(birkin completion bash)"

_birkin_profiles() {
    local profiles_dir="$HOME/.birkin/profiles"
...
_birkin_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_birkin_profiles)" -- "$cur"))
        return
    fi
...
complete -F _birkin_completion birkin
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for birkin profile names."""
    return '''#compdef birkin
# Birkin profile completion
# Add to ~/.zshrc: eval "$(birkin completion zsh)"

_birkin() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.birkin/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.birkin/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a BIRKIN_HOME path string.

    Called early in the CLI entry point, before any birkin modules
    are imported, to set the BIRKIN_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: birkin profile create {profile_name}"
        )
