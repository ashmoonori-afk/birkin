"""
Single source of truth for provider identity in Birkin.
...
2. **Birkin overlays** — transport type, auth patterns, aggregator flags,
...
# -- Birkin overlay ----------------------------------------------------------
# Birkin-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class BirkinOverlay:
    """Birkin-specific provider metadata layered on top of models.dev."""
...
BIRKIN_OVERLAYS: Dict[str, BirkinOverlay] = {
    "openrouter": BirkinOverlay(
    ...
    "openai-codex": BirkinOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": BirkinOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="BIRKIN_QWEN_BASE_URL",
    ),
    "copilot-acp": BirkinOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": BirkinOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": BirkinOverlay(
    ...
    "kimi-for-coding": BirkinOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "minimax": BirkinOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": BirkinOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": BirkinOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": BirkinOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "vercel": BirkinOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": BirkinOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": BirkinOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "birkin", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Birkin overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Birkin overlay
    ...
    overlay = BIRKIN_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + birkin extra
    ...
    if overlay is not None:
        # Birkin-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
