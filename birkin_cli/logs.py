"""``birkin logs`` — view and filter Birkin log files.
...
Usage examples::
    ...
    birkin logs                    # last 50 lines of agent.log
    birkin logs -f                 # follow agent.log in real time
    birkin logs errors             # last 50 lines of errors.log
    birkin logs gateway -n 100    # last 100 lines of gateway.log
    birkin logs --level WARNING    # only WARNING+ lines
    birkin logs --session abc123   # filter by session ID substring
    birkin logs --component tools  # only tool-related lines
    birkin logs --since 1h         # lines from the last hour
    birkin logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_birkin_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Birkin runs — try 'birkin chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_birkin_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_birkin_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_birkin_home()}/logs/")
        return

    print(f"Log files in {display_birkin_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'birkin chat' to generate logs)")
