...
- ``birkin_cli.providers``        -- canonical provider identity + overlays
- ``birkin_cli.model_normalize``  -- per-provider name formatting
...
from birkin_cli.providers import (
...
_BIRKIN_MODEL_WARNING = (
    "Nous Research Birkin 3 & 4 models are NOT agentic and are not designed "
    "for use with Birkin. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Birkin 3 / Birkin 4 chat families.
# The previous substring check (`"birkin" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``birkin-brain:qwen3-14b-ctx16k`` that just
# happen to carry "birkin" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Hermes-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   birkin-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_BIRKIN_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])birkin[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_birkin_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Birkin 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_BIRKIN_NON_AGENTIC_RE.search(model_name))


def _check_birkin_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Birkin 3/4 chat model."""
    if is_nous_birkin_non_agentic(model_name):
        return _BIRKIN_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    from birkin_cli.runtime_provider import resolve_runtime_provider
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'birkin model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'birkin doctor' — config issues detected:"
    ...
    birkin_warn = _check_birkin_model_warning(new_model)
    if birkin_warn:
        warnings.append(birkin_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from birkin_cli/models.py (OPENROUTER_MODELS,
    ...
    from birkin_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by birkin provider ID
    ...
    # --- 1. Check Birkin-mapped providers ---
    for birkin_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(birkin_id)
        ...
        model_ids = curated.get(birkin_id, [])
        total = len(model_ids)
    ...
    # --- 2. Check Birkin-only providers (nous, openai-codex, copilot, opencode-go) ---
    from birkin_cli.providers import BIRKIN_OVERLAYS
    from birkin_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Birkin provider ID.
    # BIRKIN_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Birkin IDs ("copilot").
    _mdev_to_birkin = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in BIRKIN_OVERLAYS.items():
        if pid in seen_slugs:
            continue

        # Resolve Birkin slug — e.g. "github-copilot" → "copilot"
        birkin_slug = _mdev_to_birkin.get(pid, pid)
        if birkin_slug in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                from birkin_cli.auth import _load_auth_store
                store = _load_auth_store()
                providers_store = store.get("providers", {})
                pool_store = store.get("credential_pool", {})
                if store and (
                    pid in providers_store or birkin_slug in providers_store
                    or pid in pool_store or birkin_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(birkin_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", birkin_slug, exc)
        ...
        # configured.
        if not has_creds and birkin_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    read_claude_code_credentials,
                    read_birkin_oauth_credentials,
                )
                birkin_creds = read_birkin_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (birkin_creds and birkin_creds.get("accessToken")) or \
        ...
        # Use curated list — look up by Birkin slug, fall back to overlay key
        model_ids = curated.get(birkin_slug, []) or curated.get(pid, [])
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": birkin_slug,
            "name": get_label(birkin_slug),
            "is_current": birkin_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "birkin",
        ...
        seen_slugs.add(pid)
        seen_slugs.add(birkin_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or BIRKIN_OVERLAYS (keeps /model in sync
    # with `birkin model`).
    try:
        from birkin_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
