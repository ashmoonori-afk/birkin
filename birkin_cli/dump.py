"""
Dump command for birkin CLI.

Outputs a compact, plain-text summary of the user's Birkin setup
...
from birkin_cli.config import get_birkin_home, get_env_path, get_project_root, load_config
...
def _count_skills(birkin_home: Path) -> int:
...
def _cron_summary(birkin_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = birkin_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"birkin_home:      {display_birkin_home()}")
    ...
    toolsets = config.get("toolsets", ["birkin-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(birkin_home)}")
    lines.append(f"  skills:             {_count_skills(birkin_home)}")
