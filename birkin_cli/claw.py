"""birkin claw — OpenClaw migration commands.
...
Usage:
    ...
    birkin claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    birkin claw cleanup              # Archive leftover OpenClaw directories
    birkin claw cleanup --dry-run    # Preview what would be archived
"""
...
from birkin_cli.config import get_birkin_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Birkin may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Birkin gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Birkin gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'birkin stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_birkin.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_birkin", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: birkin claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Birkin")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'birkin claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Birkin migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: birkin claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {birkin_home}")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: birkin claw migrate --yes")
            return
    ...
    # 'birkin claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: birkin claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: birkin claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  birkin claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  birkin claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  birkin config set OPENROUTER_API_KEY sk-or-v1-...")
