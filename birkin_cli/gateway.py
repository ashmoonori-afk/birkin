"""
Gateway subcommand for birkin CLI.

Handles: birkin gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_birkin_home is imported lazily at call sites to avoid ImportError
# when birkin_constants is cached from a pre-update version during `birkin update`.
from birkin_cli.setup import (
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "birkin-gateway*",
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``birkin update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Birkin profile are returned.
    """
    _exclude = exclude_pids or set()
    pids = [pid for pid in _get_service_pids() if pid not in _exclude]
    patterns = [
        "birkin_cli.main gateway",
        "birkin_cli.main --profile",
        "birkin_cli.main -p",
        "birkin_cli/main.py gateway",
        "birkin_cli/main.py --profile",
        "birkin_cli/main.py -p",
        "birkin gateway",
        "gateway/run.py",
    ]
    current_home = str(get_birkin_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"BIRKIN_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "BIRKIN_HOME=" in command and f"BIRKIN_HOME={current_home}" not in command:
            return False
        return True
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (BIRKIN_HOME-scoped).
...
from birkin_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "birkin-gateway"
SERVICE_DESCRIPTION = "Birkin Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current BIRKIN_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (BIRKIN_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from birkin_constants import get_default_birkin_root
    home = get_birkin_home().resolve()
    default = get_default_birkin_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary BIRKIN_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(birkin_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when BIRKIN_HOME is a named profile.

    For ``~/.birkin/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        birkin_home: Optional explicit BIRKIN_HOME path. Defaults to the current
            ``get_birkin_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from birkin_constants import get_default_birkin_root
    home = Path(birkin_home or str(get_birkin_home())).resolve()
    default = get_default_birkin_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this BIRKIN_HOME.

    Default ``~/.birkin`` returns ``birkin-gateway`` (backward compatible).
    Profile ``~/.birkin/profiles/coder`` returns ``birkin-gateway-coder``.
    Any other BIRKIN_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    birkin gateway uninstall")
    print_info("    sudo birkin gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Birkin can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo birkin gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo birkin gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo birkin gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Birkin often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.birkin`` → ``ai.birkin.gateway.plist`` (backward compatible).
    Profile ``~/.birkin/profiles/coder`` → ``ai.birkin.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.birkin.gateway-{suffix}" if suffix else "ai.birkin.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.birkin/birkin-agent  -> /home/alice/.birkin/birkin-agent
      /opt/birkin                 -> /opt/birkin  (kept as-is)
...
def _birkin_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current BIRKIN_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_birkin_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.birkin                    → /home/alice/.birkin
      /root/.birkin/profiles/coder     → /home/alice/.birkin/profiles/coder
      /opt/custom-birkin               → /opt/custom-birkin  (kept as-is)
    """
    current_birkin = get_birkin_home().resolve()
    current_default = (Path.home() / ".birkin").resolve()
    target_default = Path(target_home_dir) / ".birkin"

    # Default ~/.birkin → remap to target user's default
    if current_birkin == current_default:
        return str(target_default)

    # Profile or subdir of ~/.birkin → preserve the relative structure
    try:
        relative = current_birkin.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.birkin) — keep as-is
        return str(current_birkin)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        birkin_home = _birkin_home_for_target_user(home_dir)
        profile_arg = _profile_arg(birkin_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m birkin_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="BIRKIN_HOME={birkin_home}"
Restart=on-failure
RestartSec=30
...
"""
    ...
    birkin_home = str(get_birkin_home().resolve())
    profile_arg = _profile_arg(birkin_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Birkin install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}birkin gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}birkin gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}birkin gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}birkin gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}birkin gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.birkin.gateway-{suffix}" if suffix else "ai.birkin.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Birkin install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  birkin gateway status             # Check status")
...
def launchd_stop():
    ...
    # `birkin gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Birkin install")
    else:
        print("⚠ Service definition is stale relative to the current Birkin install")
        print("  Run: birkin gateway start")
    ...
    if deep:
        log_file = get_birkin_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Birkin will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @birkin:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. birkin) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Birkin delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Birkin agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Birkin will use (e.g., birkin@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Birkin connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: birkin pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_birkin_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'birkin pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: birkin pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'birkin gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from birkin_cli.main import cmd_whatsapp
    import argparse
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Birkin will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Birkin will store the returned account_id/token in ~/.birkin/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `birkin gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_birkin_home())))
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `birkin pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `birkin pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"BirkinAgent\"")
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: birkin gateway")
                except subprocess.CalledProcessError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  birkin gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: birkin gateway run")
                print_info("  For persistence:   tmux new -s birkin 'birkin gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: birkin gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup birkin gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: birkin gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'birkin gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: birkin gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: birkin gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s birkin 'birkin gateway run'")
                print()
            ...
            print()
            print("  birkin gateway run                              # direct foreground")
            print("  tmux new -s birkin 'birkin gateway run'         # persistent via tmux")
            print("  nohup birkin gateway run > ~/.birkin/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: birkin gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: birkin gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: birkin gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: birkin gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: birkin gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  birkin gateway install")
                    print("  sudo birkin gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  birkin gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup birkin gateway run > ~/.birkin/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s birkin 'birkin gateway run'         # persistent via tmux")
                    print("  nohup birkin gateway run > ~/.birkin/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  birkin gateway install  # Install as user service")
                    print("  sudo birkin gateway install --system  # Install as boot-time system service")
