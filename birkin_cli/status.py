"""
Status command for birkin CLI.

Shows the status of all Birkin components.
"""
...
from birkin_cli.auth import AuthError, resolve_provider
from birkin_cli.colors import Colors, color
from birkin_cli.config import get_env_path, get_env_value, get_birkin_home, load_config
from birkin_cli.models import provider_label
from birkin_cli.nous_subscription import get_nous_subscription_features
from birkin_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all Birkin components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Birkin Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from birkin_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: birkin model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: birkin model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: birkin model)"
    ...
    if not terminal_env:
        ...
        # (birkin status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    if _is_termux():
        try:
            from birkin_cli.gateway import find_gateway_pids
        ...
        if gateway_pids:
            ...
            print("  Start with:   birkin gateway")
        ...
        from birkin_cli.gateway import get_launchd_label
        try:
            result = subprocess.run(
    ...
    jobs_file = get_birkin_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_birkin_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'birkin doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'birkin setup' to configure", Colors.DIM))
    print()
