...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or birkin -c.",
    ...
    "Ctrl+Z suspends Birkin to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "birkin -c resumes your most recent CLI session. birkin -c \"project name\" resumes by title.",
    "birkin -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "birkin -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "birkin chat -t web,terminal enables only specific toolsets for a focused session.",
    "birkin chat -s github-pr-workflow preloads a skill at launch.",
    "birkin chat -q \"query\" runs a single non-interactive query and exits.",
    "birkin chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "birkin chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "birkin --yolo bypasses all dangerous command approval prompts for the entire session.",
    "birkin chat --source telegram tags the session for filtering in birkin sessions list.",
    "birkin -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "birkin doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "birkin dump outputs a compact setup summary — great for bug reports.",
    "birkin config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "birkin config edit opens config.yaml in your default editor.",
    "birkin config check scans for missing or stale configuration options.",
    "birkin sessions browse opens an interactive session picker with search.",
    "birkin sessions stats shows session counts by platform and database size.",
    "birkin sessions prune --older-than 30 cleans up old sessions.",
    "birkin skills search react --source skills-sh searches the skills.sh public directory.",
    "birkin skills check scans installed hub skills for upstream updates.",
    "birkin skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "birkin skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "birkin mcp add github --command npx adds MCP servers from the command line.",
    "birkin mcp serve runs Birkin itself as an MCP server for other agents.",
    "birkin auth add lets you add multiple API keys for credential pool rotation.",
    "birkin completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "birkin logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "birkin backup creates a zip backup of your entire Birkin home directory.",
    "birkin profile create coder creates an isolated profile that becomes its own command.",
    "birkin profile create work --clone copies your current config and keys to a new profile.",
    "birkin update syncs new bundled skills to ALL profiles automatically.",
    "birkin gateway install sets up Birkin as a system service (systemd/launchd).",
    "birkin memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "birkin webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as birkin -w).",
    ...
    "BIRKIN_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Birkin tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'birkin profile create coder' creates the 'coder' command.",
    "birkin profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Birkin prints a resume command with session ID and stats.",
    "birkin sessions export backup.jsonl exports all sessions for backup or analysis.",
    "birkin -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "birkin skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .birkin/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: birkin cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Birkin runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "birkin gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Birkin via a local macOS server.",
    ...
    "Context files (.birkin.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.birkin/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Birkin loads project context from .birkin.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — birkin tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.birkin/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "birkin chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "birkin plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "birkin acp runs Birkin as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "BIRKIN_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "birkin login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "birkin status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.birkin/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.birkin/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Birkin your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.birkin/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Birkin auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.birkin/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at BIRKIN_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "BIRKIN_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in BIRKIN_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.birkin/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'birkin profile create ls' would be rejected.",
    "birkin profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "birkin claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "BIRKIN_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via birkin tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.birkin/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.birkin/skills/.hub/quarantine/ holds skills pending security review.",
]
