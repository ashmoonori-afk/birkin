"""
Birkin Plugin System
====================

Discovers, loads, and manages plugins from three sources:

1. **User plugins**   – ``~/.birkin/plugins/<name>/``
2. **Project plugins** – ``./.birkin/plugins/<name>/`` (opt-in via
   ``BIRKIN_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``birkin_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "birkin_agent.plugins"

_NS_PARENT = "birkin_plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``birkin honcho ...``).
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.birkin/plugins/)
        user_dir = get_birkin_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.birkin/plugins/)
        if _env_enabled("BIRKIN_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".birkin" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``birkin_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``birkin tools`` TUI so plugin-provided toolsets appear
