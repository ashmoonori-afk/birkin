"""
Doctor command for birkin CLI.

Diagnoses issues with Birkin setup.
"""
...
from birkin_cli.config import get_project_root, get_birkin_home, get_env_path
from birkin_constants import display_birkin_home

PROJECT_ROOT = get_project_root()
BIRKIN_HOME = get_birkin_home()
_DHH = display_birkin_home()  # user-facing display path (e.g. ~/.birkin or ~/.birkin/profiles/coder)

# Load environment variables from ~/.birkin/.env so API key checks work
...
from birkin_constants import OPENROUTER_MODELS_URL
...
from birkin_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.birkin/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `birkin`.
    os.environ.setdefault("BIRKIN_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 Birkin Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.birkin/.env (primary location for user config)
    env_path = BIRKIN_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'birkin setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'birkin setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'birkin setup' to create one")
                issues.append("Run 'birkin setup' to create .env")
    
    # Check ~/.birkin/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = BIRKIN_HOME / 'config.yaml'
    ...
    if config_path.exists():
        try:
            from birkin_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'birkin setup' to migrate config")
                else:
                    issues.append("Run 'birkin doctor --fix' or 'birkin setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'birkin doctor --fix'")
        except Exception:
            pass
    ...
    try:
        from birkin_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    if birkin_home.exists():
        check_ok(f"{_DHH} directory exists")
    else:
        if should_fix:
            birkin_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = birkin_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Birkin a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Birkin Persona\n\n"
                "<!-- Edit this file to customize how Birkin communicates. -->\n\n"
    ...
    memories_dir = birkin_home / "memories"
    ...
    state_db_path = birkin_home / "state.db"
    ...
    wal_path = birkin_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'birkin doctor --fix' to checkpoint")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'birkin setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    hub_dir = BIRKIN_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: birkin skills list)")
    ...
    try:
        ...
        _mem_cfg_path = BIRKIN_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: birkin memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: birkin memory setup")
                issues.append("No Honcho API key — run 'birkin memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run birkin memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: birkin memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: birkin memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from birkin_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "birkin -p" in content:
                            _m = _re.search(r"birkin -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'birkin doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
