"""
Interactive setup wizard for Birkin.
...
Config files are stored in ~/.birkin/ for easy access.
"""
...
_DOCS_BASE = "https://birkin-agent.nousresearch.com/docs"
...
# display_birkin_home imported lazily at call sites (stale-module safety during birkin update)

from birkin_cli.colors import Colors, color
...
from birkin_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Birkin Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Birkin using environment variables or config commands:")
    print_info("  birkin config set model.provider custom")
    print_info("  birkin config set model.base_url http://localhost:8080/v1")
    print_info("  birkin config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'birkin setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0) -> int:
    ...
    from birkin_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from birkin_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'birkin setup')")


def _print_setup_summary(config: dict, birkin_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'birkin setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'birkin setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'birkin setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'birkin setup tools' to configure them,"
        )
        from birkin_constants import display_birkin_home as _dhh
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {birkin_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('birkin setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('birkin setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('birkin setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('birkin setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('birkin setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('birkin config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('birkin config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('birkin config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('birkin', Colors.GREEN)}              Start chatting")
    print(f"   {color('birkin gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('birkin doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `birkin tools` and `birkin setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``birkin model``)
    ...
    provider added to ``birkin model`` is automatically available here.
    ...
    # Delegate to the shared birkin model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from birkin_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: birkin model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from birkin_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Birkin can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Birkin can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'birkin setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.birkin/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'birkin setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Birkin runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Birkin via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("BIRKIN_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `birkin setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("BIRKIN_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("BIRKIN_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Birkin delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://birkin-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Birkin delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Birkin delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'birkin whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'birkin whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from birkin_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from birkin_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from birkin_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from birkin_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from birkin_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from birkin_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from birkin_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from birkin_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_qqbot():
    ...
    print_info("Connects Birkin to QQ via the Official QQ Bot API (v2).")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Birkin to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from birkin_cli.gateway import _PLATFORMS
    qq_platform = next((p for p in _PLATFORMS if p["key"] == "qqbot"), None)
    if qq_platform:
        from birkin_cli.gateway import _setup_standard_platform
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://birkin-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://birkin-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  birkin config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Birkin from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'birkin setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     birkin config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: birkin gateway install")
            else:
                print_info("  You can install later: birkin gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo birkin gateway install --system")
                print_info("  Or run in foreground:  birkin gateway")
        else:
            from birkin_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   birkin gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   birkin gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `birkin setup tools` and `birkin tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from birkin_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from birkin_cli.auth import get_active_provider
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_birkin.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_birkin migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_birkin", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Birkin.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Birkin to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Birkin at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Birkin at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Birkin at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Birkin at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Birkin equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Birkin config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Birkin.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Birkin's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(birkin_home: Path) -> bool:
    ...
    print_info("Birkin can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: birkin claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=birkin_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: birkin claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Birkin configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Birkin config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Birkin (use birkin claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      birkin setup           — full or quick (auto-detected)
      birkin setup model     — just model/provider
      birkin setup tts       — just text-to-speech
      birkin setup terminal  — just terminal backend
      birkin setup gateway   — just messaging platforms
      birkin setup tools     — just tool configuration
      birkin setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Birkin Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Birkin Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Birkin installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Birkin configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, birkin_home)
            return
        elif choice == 1:
            # Full setup — fall through to run all sections
            pass
        elif choice == 7:
            print_info("Exiting. Run 'birkin setup' again when ready.")
            return
        elif 2 <= choice <= 6:
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, birkin_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(birkin_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Birkin?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, birkin_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {birkin_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'birkin config edit'")
...
def _resolve_birkin_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``birkin chat`` in a fresh process."""
    birkin_bin = shutil.which("birkin")
    if birkin_bin:
        return [birkin_bin, "chat"]

    try:
        if importlib.util.find_spec("birkin_cli") is not None:
            return [sys.executable, "-m", "birkin_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch birkin chat now?", True):
        return

    chat_argv = _resolve_birkin_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch Birkin automatically. Run 'birkin chat' manually.")
        return

    os.execvp(chat_argv[0], chat_argv)


def _run_first_time_quick_setup(config: dict, birkin_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'birkin setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    birkin setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  birkin setup gateway")
    print()
...
def _run_quick_setup(config: dict, birkin_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'birkin setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Birkin to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'birkin setup gateway'.")
