...
Thank you for contributing to Birkin! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want Birkin to work everywhere.
...
Bundled skills (in `skills/`) ship with every Birkin install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `birkin skills browse` (labeled "official") and install it with `birkin skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the Birkin community Discord. Users can install it with `birkin skills install`.
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.birkin/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/birkin" ~/.local/bin/birkin
...
├── cli.py                    # BirkinCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (birkin-cli, birkin-telegram, etc.)
├── birkin_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── skills/                   # Bundled skills (copied to ~/.birkin/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── docs/site/website/        # Documentation site
├── docs/site/landingpage/    # Landing page
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.birkin/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.birkin/`)

| Path | Purpose |
|------|---------|
| `~/.birkin/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.birkin/.env` | API keys and secrets |
| `~/.birkin/auth.json` | OAuth credentials (Nous Portal) |
| `~/.birkin/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.birkin/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.birkin/state.db` | SQLite session database |
| `~/.birkin/sessions/` | JSON session logs |
| `~/.birkin/cron/` | Scheduled job data |
| `~/.birkin/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`birkin_state.py`) with full-text search and unique session titles. JSON logs go to `~/.birkin/sessions/`.
...
Four fields are supported under `metadata.birkin`:
...
The user may skip setup and keep loading the skill. Birkin only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `birkin setup` or update `~/.birkin/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Birkin tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `birkin --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Birkin uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.birkin/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `birkin_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `birkin_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Birkin runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
Birkin has terminal access. Security matters.
...
2. **Test manually**: Run `birkin` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/MoonGwanghoon/birkin/issues)
- Include: OS, Python version, Birkin version (`birkin version`), full error traceback
