...
sidebar_position: 1
title: "Architecture"
description: "Birkin internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Birkin internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ birkin_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # BirkinCLI — interactive terminal UI (~10,000 lines)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── birkin_state.py           # SQLite session/state database with FTS5
├── birkin_constants.py       # BIRKIN_HOME, profile-aware paths
...
│   ├── main.py               # Entry point — all `birkin` subcommands (~6,000 lines)
...
│   ├── skills_config.py      # birkin skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # birkin tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # birkin gateway start/stop
│
...
```text
User input → BirkinCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .birkin.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.birkin/plugins/` (user), `.birkin/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `birkin plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-birkin-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Birkin as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`birkin -p <name>`) gets its own BIRKIN_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
