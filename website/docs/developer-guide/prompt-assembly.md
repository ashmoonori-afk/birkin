...
description: "How Birkin builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Birkin deliberately separates:
...
1. agent identity — `SOUL.md` from `BIRKIN_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.birkin/SOUL.md)
You are Birkin, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.birkin/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_birkin_md(cwd_path)       # 1. .birkin.md / BIRKIN.md (walks to git root)
    ...
    # SOUL.md from BIRKIN_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.birkin.md`, `BIRKIN.md` | CWD up to git root | Birkin-native project config |
...
- **YAML frontmatter stripped** — `.birkin.md` frontmatter is removed (reserved for future config overrides)
...
1. `.birkin.md` / `BIRKIN.md` (walks to git root)
