...
The messaging gateway is the long-running process that connects Birkin to 14+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `birkin_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.birkin/.env` | API keys, bot tokens, platform credentials |
| `~/.birkin/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.birkin/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `birkin gateway start` / `birkin gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.birkin/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `birkin gateway stop` stops only the current profile's gateway. `birkin gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
