...
The ACP adapter wraps Birkin' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
birkin acp / birkin-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.birkin/.env
  -> configure stderr logging
  -> construct BirkinACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> Birkin `once`
- `allow_always` -> Birkin `always`
- reject options -> Birkin `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Birkin tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["birkin-acp"])
...
Instead it reuses Birkin' runtime resolver:

- `acp_adapter/auth.py`
- `birkin_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Birkin provider/credentials.
...
- `toolsets.py` — `birkin-acp` toolset definition
- `birkin_cli/main.py` — `birkin acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `birkin-acp` script
