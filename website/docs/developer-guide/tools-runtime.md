...
Birkin tools are self-registering functions grouped into toolsets and executed through a central registry/dispatch system.
...
2. **Plugin tools** — `birkin_cli.plugins.discover_plugins()` loads user/project/pip plugins that may register additional tools.
...
Toolsets are named bundles of tools. Birkin resolves them through:

- explicit enabled/disabled toolset lists
- platform presets (`birkin-cli`, `birkin-telegram`, etc.)
- dynamic MCP toolsets
- curated special-purpose sets like `birkin-acp`
