...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Birkin-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Birkin includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Birkin/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- BirkinAgentBaseEnv
    BirkinAgentBaseEnv <|-- TerminalTestEnv
    BirkinAgentBaseEnv <|-- BirkinSweEnv
    BirkinAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The birkin-agent layer (`environments/birkin_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls birkin-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `BirkinAgentLoop` and scores the result
...
Your environment inherits from `BirkinAgentBaseEnv` and implements five methods:
...
`BirkinAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as birkin-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any birkin-agent tool |
...
parser = get_parser("birkin")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `birkin`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "birkin-agent[yc-bench]"
...
```bash
python environments/birkin_swe_env/birkin_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"birkin"`, `"qwen"`, `"mistral"`)
...
```python
from environments.birkin_base_env import BirkinAgentBaseEnv, BirkinAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(BirkinAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(BirkinAgentBaseEnv):
    name = "my-env"
...
### BirkinAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which birkin toolsets to enable |
...
| `tool_call_parser` | `str` | `"birkin"` | Parser for Phase 2 raw output |
...
- [Modal](https://modal.com) account and CLI: `pip install "birkin-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "birkin-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── birkin_base_env.py          # Abstract base class (BirkinAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (BirkinAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── birkin_parser.py        # Birkin/ChatML <tool_call> format
...
├── birkin_swe_env/             # SWE-bench training environment
│
