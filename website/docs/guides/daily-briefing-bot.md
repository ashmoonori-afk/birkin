...
2. **Birkin spins up** a fresh agent session with your prompt
...
- **Birkin installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.birkin/cron/output/` and you can read them anytime.
:::
...
Birkin will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Birkin
```
...
Just tell Birkin what you want:
...
Birkin will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Birkin to delegate each topic to a sub-agent:
...
Birkin will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
