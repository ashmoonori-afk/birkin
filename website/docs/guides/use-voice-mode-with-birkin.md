---
sidebar_position: 8
title: "Use Voice Mode with Birkin"
description: "A practical guide to setting up and using Birkin voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Birkin
...
- you want Birkin sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Birkin.
...
## Step 1: make sure normal Birkin works first
...
Birkin supports both local and cloud speech stacks.
...
### If you use `birkin setup`

If you choose NeuTTS in the setup wizard, Birkin checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Birkin transcribes and responds
...
- asking Birkin to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Birkin loop.

## Tuning CLI behavior

### Silence threshold

If Birkin starts/stops too aggressively, tune:
...
Birkin stays a normal chat bot, but can speak replies.
...
- you want Birkin to function like a portable research or ops assistant
...
Birkin joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Birkin detects speech boundaries
- transcripts are posted in the associated text channel
- Birkin responds in text and audio
...
1. get text Birkin working
2. install `birkin-agent[voice]`
