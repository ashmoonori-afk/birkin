---
sidebar_position: 6
title: "Use MCP with Birkin"
description: "A practical guide to connecting MCP servers to Birkin, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Birkin

This guide shows how to actually use MCP with Birkin in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Birkin tool
- you want Birkin to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Birkin to internal APIs, databases, or company systems without modifying Birkin core

Do not use MCP when:
- a built-in Birkin tool already solves the job well
...
- Birkin remains the agent
- MCP servers contribute tools
- Birkin discovers those tools at startup or reload time
...
If you installed Birkin with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Birkin banner/status should show MCP integration when configured
- ask Birkin what tools it has available
...
There are two categories of MCP-exposed functionality in Birkin:
...
2. Birkin-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Birkin will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Birkin to reason over a bounded workspace.
...
Now Birkin can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Birkin core.
...
Because Birkin now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
