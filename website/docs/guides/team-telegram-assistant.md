...
This tutorial walks you through setting up a Telegram bot powered by Birkin that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Birkin installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.birkin/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Birkin itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Birkin Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_birkin_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Birkin. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Birkin Gateway...
...
# View live logs
journalctl --user -u birkin-gateway -f
...
birkin gateway start
birkin gateway stop
tail -f ~/.birkin/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `birkin gateway install` to update the plist.
:::
...
```bash
birkin gateway stop && birkin gateway start
```
...
birkin pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.birkin/.env`:
...
Control how much detail the bot shows when using tools. In `~/.birkin/config.yaml`:
...
Customize how the bot communicates by editing `~/.birkin/SOUL.md`:

For a full guide, see [Use SOUL.md with Birkin](/docs/guides/use-soul-with-birkin).
...
```bash
# From the CLI
birkin cron list          # View all scheduled jobs
birkin cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u birkin-gateway` (Linux) or `~/.birkin/logs/gateway.log` (macOS) |
| Cron job output | `~/.birkin/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.birkin/cron/jobs.json` |
| Pairing data | `~/.birkin/pairing/` |
| Session history | `~/.birkin/sessions/` |
