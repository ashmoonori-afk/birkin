---
sidebar_position: 7
title: "Use SOUL.md with Birkin"
description: "How to use SOUL.md to shape Birkin's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Birkin

`SOUL.md` is the **primary identity** for your Birkin instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Birkin to feel like the same assistant every time you talk to it — or if you want to replace the Birkin persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Birkin should be
- what Birkin should avoid stylistically
- how Birkin should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Birkin is and how Birkin speaks
...
Birkin now uses only the global SOUL file for the current instance:

```text
~/.birkin/SOUL.md
```

If you run Birkin with a custom home directory, it becomes:
...
Birkin automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Birkin does not overwrite it
- if the file exists but is empty, Birkin adds nothing from it to the prompt

## How Birkin uses it

When Birkin starts a session, it reads `SOUL.md` from `BIRKIN_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Birkin feels.
...
Birkin already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Birkin should sound.

# Avoid
What Birkin should not do.

# Defaults
How Birkin should behave when ambiguity appears.
```
...
Then restart Birkin or start a new session.
...
### I edited SOUL.md but Birkin still sounds the same

Check:
- you edited `~/.birkin/SOUL.md` or `$BIRKIN_HOME/SOUL.md`
...
### Birkin is ignoring parts of my SOUL.md
