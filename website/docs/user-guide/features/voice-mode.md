...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Birkin — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Birkin supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Birkin](/docs/guides/use-voice-mode-with-birkin).

## Prerequisites

Before using voice features, make sure you have:

1. **Birkin installed** — `pip install birkin-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `birkin model` or set your preferred provider credentials in `~/.birkin/.env`
3. **A working base setup** — run `birkin` to verify the agent responds to text before enabling voice

:::tip
The `~/.birkin/` directory and default `config.yaml` are created automatically the first time you run `birkin`. You only need to create `~/.birkin/.env` manually for API keys.
:::
...
pip install "birkin-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "birkin-agent[all]"
```
...
```bash
birkin                # Start the interactive CLI
```
...
1. Start the CLI with `birkin` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.birkin/config.yaml` (default: `ctrl+b`).
:::
...
```bash
birkin gateway        # Start the gateway (connects to configured platforms)
birkin gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@birkinbot hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.birkin/.env`:
```bash
...
```bash
birkin gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.birkin/.env`
...
- Review gateway logs: `tail -f ~/.birkin/logs/gateway.log`
