...
If the file is empty, whitespace-only, or cannot be read, Birkin falls back to a built-in default identity ("You are Birkin, an intelligent AI assistant..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
