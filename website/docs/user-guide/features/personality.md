...
description: "Customize Birkin's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Birkin's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `BIRKIN_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Birkin is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Birkin now seeds a default `SOUL.md` automatically in:

```text
~/.birkin/SOUL.md
```

More precisely, it uses the current instance's `BIRKIN_HOME`, so if you run Birkin with a custom home directory, it will use:
...
- Birkin creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Birkin loads `SOUL.md` only from `BIRKIN_HOME`
- Birkin does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Birkin falls back to a built-in default identity
...
If Birkin loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `BIRKIN_HOME`, the personality belongs to the Birkin instance itself.

That also makes it easier to teach users:
- "Edit `~/.birkin/SOUL.md` to change Birkin' default personality."
...
- how Birkin should handle uncertainty, disagreement, or ambiguity
...
## What Birkin injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Birkin falls back to a built-in default identity ("You are Birkin, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Birkin ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Birkin, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Birkin its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.birkin/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.birkin/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-birkin)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Birkin speaks
- `display.skin` and `/skin` affect how Birkin looks in the terminal
