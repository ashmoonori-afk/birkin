...
Birkin ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
birkin memory setup      # interactive picker + configuration
birkin memory status     # check what's active
birkin memory off        # disable external provider
```

You can also select the active memory provider via `birkin plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.birkin/config.yaml`:
...
When a memory provider is active, Birkin automatically:
...
**Setup Wizard:**
```bash
birkin honcho setup        # (legacy command) 
# or
birkin memory setup        # select "honcho"
```

**Config:** `$BIRKIN_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$BIRKIN_HOME/honcho.json` > `~/.birkin/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/birkin-ai/birkin-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/birkin).
...
:::tip Migrating from `birkin honcho`
If you previously used `birkin honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-agent / Profiles:**

Each Birkin profile gets its own Honcho AI peer while sharing the same workspace -- all profiles see the same user representation, but each agent builds its own identity and observations.

```bash
birkin profile create coder --clone   # creates honcho peer "coder", inherits config from default
```

What `--clone` does: creates a `birkin.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The peer is eagerly created in Honcho so it exists before first message.

For profiles created before Honcho was set up:

```bash
birkin honcho sync   # scans all profiles, creates host blocks for any missing ones
```

This inherits settings from the default `birkin` host block and creates new AI peers for each profile. Idempotent -- skips profiles that already have a host block.
...
See the [config reference](https://github.com/birkin-ai/birkin-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/birkin).
...
birkin memory setup    # select "openviking"
# Or manually:
birkin config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.birkin/.env
```
...
**Setup:**
```bash
birkin memory setup    # select "mem0"
# Or manually:
birkin config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.birkin/.env
```

**Config:** `$BIRKIN_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `birkin-user` | User identifier |
| `agent_id` | `birkin` | Agent identifier |
...
**Setup:**
```bash
birkin memory setup    # select "hindsight"
# Or manually:
birkin config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.birkin/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p birkin ui start`

**Config:** `$BIRKIN_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `birkin` | Memory bank identifier |
...
See [plugin README](https://github.com/NousResearch/birkin-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
birkin memory setup    # select "holographic"
# Or manually:
birkin config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.birkin-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$BIRKIN_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
birkin memory setup    # select "retaindb"
# Or manually:
birkin config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.birkin/.env
```
...
# Then configure Birkin
birkin memory setup    # select "byterover"
# Or manually:
birkin config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$BIRKIN_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
birkin memory setup    # select "supermemory"
# Or manually:
birkin config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.birkin/.env
```

**Config:** `$BIRKIN_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `birkin` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `birkin-{identity}` → `birkin-coder`) to isolate memories per Birkin profile
...
- **Local storage providers** (Holographic, ByteRover) use `$BIRKIN_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$BIRKIN_HOME/` so each profile has its own credentials
