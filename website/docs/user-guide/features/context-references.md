...
Type `@` followed by a reference to inject content directly into your message. Birkin expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Birkin env: `$BIRKIN_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$BIRKIN_HOME/skills/.hub/`
