...
Connect Birkin to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Birkin and the BlueBubbles server
...
Or set environment variables directly in `~/.birkin/.env`:
...
When someone messages your iMessage, Birkin automatically sends them a pairing code. Approve it with:
```bash
birkin pairing approve bluebubbles <CODE>
```
Use `birkin pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.birkin/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.birkin/.env`):
```bash
...
Birkin will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Birkin
Birkin → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Birkin sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — Birkin resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `birkin logs gateway` for webhook errors (or `birkin logs -f` to follow in real-time)
