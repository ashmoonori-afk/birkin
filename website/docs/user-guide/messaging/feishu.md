...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Birkin as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Birkin integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Birkin opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Birkin responds to every message. |
| Group chats | Birkin responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Birkin will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `birkin gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when Birkin runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Birkin behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Birkin starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
Add the following to `~/.birkin/.env`:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Birkin responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Birkin responds to @mentions from any user in any group. |
| `allowlist` | Birkin only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Birkin ignores all group messages entirely. |
...
|---------|---------|---------|
| Quiet period | `BIRKIN_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `BIRKIN_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `BIRKIN_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `BIRKIN_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.birkin/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `BIRKIN_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `BIRKIN_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `BIRKIN_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `BIRKIN_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `BIRKIN_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `BIRKIN_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `birkin gateway setup` |
| `Another local Birkin gateway is already using this Feishu app_id` | Only one Birkin instance can use the same app_id at a time. Stop the other gateway first. |
...
Feishu / Lark uses the `birkin-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
