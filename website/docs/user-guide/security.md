...
Birkin is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Birkin checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.birkin/config.yaml`:
...
1. **CLI flag**: Start a session with `birkin --yolo` or `birkin chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `BIRKIN_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `BIRKIN_YOLO_MODE` environment variable which is checked before every command execution.
...
Configure the timeout in `~/.birkin/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.birkin/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.birkin/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` birkin/gateway | Self-termination prevention |
...
The `BIRKIN_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.birkin/config.yaml`:
...
:::tip
Use `birkin config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Birkin controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.birkin/.env`:
...
For more flexible authorization, Birkin includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `birkin pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.birkin/config.yaml`:
...
birkin pairing approve telegram ABC12DEF

# Revoke a user's access
birkin pairing revoke telegram 123456789

# Clear all pending codes
birkin pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.birkin/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Birkin applies strict security hardening to every container.
...
Container resources are configurable in `~/.birkin/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.birkin/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `BIRKIN_HOME`. Skills declare these in frontmatter:
...
When loaded, Birkin checks if these files exist in the active profile's `BIRKIN_HOME` and registers them for mounting:
...
Paths are relative to `~/.birkin/`. Files are mounted to `/root/.birkin/` inside the container.
...
| **terminal** (local) | Blocks explicit Birkin infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Birkin infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Birkin integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
4. **Store secrets securely** — keep API keys in `~/.birkin/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.birkin/logs/` for unauthorized access attempts
10. **Keep updated** — run `birkin update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.birkin/.env
