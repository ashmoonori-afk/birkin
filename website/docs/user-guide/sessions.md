...
Birkin automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.birkin/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.birkin/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`birkin` or `birkin chat`) |
...
birkin -c "my project"   # → resumes "my project #3"
```
...
birkin --resume 20250305_091523_a1b2c3d4
birkin -r 20250305_091523_a1b2c3d4

# Resume by title
birkin --resume "refactoring auth"

# Or with the chat subcommand
birkin chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `birkin sessions list`.

### Conversation Recap on Resume

When you resume a session, Birkin displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Birkin session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.birkin/config.yaml`:
...
Birkin automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `birkin sessions list` or `birkin sessions browse`.
...
```bash
birkin sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Birkin creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`birkin -c "my project"`), it automatically picks the most recent session in the lineage.
...
Birkin provides a full set of session management commands via `birkin sessions`:
...
# Filter by platform
birkin sessions list --source telegram

# Show more sessions
birkin sessions list --limit 50
```
...
birkin sessions export backup.jsonl

# Export sessions from a specific platform
birkin sessions export telegram-history.jsonl --source telegram

# Export a single session
birkin sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
birkin sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
birkin sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
birkin sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
birkin sessions prune

# Custom age threshold
birkin sessions prune --older-than 30

# Only prune sessions from a specific platform
birkin sessions prune --source telegram --older-than 60

# Skip confirmation
birkin sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`birkin insights`](/docs/reference/cli-commands#birkin-insights).
...
When Birkin cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Birkin uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Birkin in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.birkin/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.birkin/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.birkin/sessions/sessions.json` | Maps session keys to active session IDs |
...
birkin sessions delete <session_id>
