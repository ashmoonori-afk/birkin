...
title: "Git Worktrees"
description: "Run multiple Birkin agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Birkin is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Birkin so each session has a clean, isolated working directory.

## Why Use Worktrees with Birkin?

Birkin treats the **current working directory** as the project root:

- CLI: the directory where you run `birkin` or `birkin chat`
...
git worktree add ../repo-feature feature/birkin-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/birkin-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Birkin there:

```bash
cd ../repo-feature

# Start Birkin in the worktree
birkin
```
...
git worktree add ../repo-experiment-a feature/birkin-a
git worktree add ../repo-experiment-b feature/birkin-b
```
...
- Works on its own branch (`feature/birkin-a` vs `feature/birkin-b`).
...
- Birkin checkpoint data under `~/.birkin/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Birkin experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/birkin-checkpoints-docs`, `feature/birkin-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Birkin from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `birkin -w` (Automatic Worktree Mode)

Birkin has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `birkin/birkin-<hash>`).
...
```bash
birkin -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `birkin -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Birkin session its own clean checkout.
