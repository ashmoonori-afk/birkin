...
description: "Run Birkin directly on an Android phone with Termux"
---

# Birkin on Android with Termux

This is the tested path for running Birkin directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Birkin from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `birkin` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `birkin` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/birkin" "$PREFIX/bin/birkin"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `birkin` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.birkin/.env`.
...
### `birkin doctor` says ripgrep or Node is missing
