...
sidebar_position: 2
title: "Installation"
description: "Install Birkin on Linux, macOS, WSL2, or Android via Termux"
---

# Installation

Get Birkin up and running in under two minutes with the one-line installer, or follow the manual steps for full control.
...
:::warning Windows
Native Windows is **not supported**. Please install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Birkin from there. The install command above works inside WSL2.
:::

### What the Installer Does

The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `birkin` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
birkin gateway setup  # Set up messaging platforms
birkin config set     # Set individual config values
birkin setup          # Or run the full setup wizard to configure everything at once
```
...
:::tip
You do **not** need to activate the venv to use `birkin`. The entry point has a hardcoded shebang pointing to the venv Python, so it works globally once symlinked.
:::
...
mkdir -p ~/.birkin/{cron,sessions,logs,memories,skills,pairing,hooks,image_cache,audio_cache,whatsapp/session}
...
Open `~/.birkin/.env` and add at minimum an LLM provider key:
...
```bash
birkin config set OPENROUTER_API_KEY sk-or-v1-your-key-here
```

### Step 8: Add `birkin` to Your PATH
...
```bash
birkin model       # Select your LLM provider and model
```

### Step 10: Verify the Installation

```bash
birkin version    # Check that the command is available
birkin doctor     # Run diagnostics to verify everything is working
birkin status     # Check your configuration
birkin chat -q "Hello! What tools do you have available?"
```
...
# Make birkin available globally
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/birkin" ~/.local/bin/birkin
...
| Problem | Solution |
|---------|----------|
| `birkin: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
| `API key not set` | Run `birkin model` to configure your provider, or `birkin config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `birkin config check` then `birkin config migrate` |

For more diagnostics, run `birkin doctor` — it will tell you exactly what's missing and how to fix it.
