...
description: "Official optional skills shipped with birkin-agent — install via birkin skills install official/<category>/<skill>"
---

# Optional Skills Catalog

Official optional skills ship with the birkin-agent repository under `optional-skills/` but are **not active by default**. Install them explicitly:
...
```bash
birkin skills install official/blockchain/solana
birkin skills install official/mlops/flash-attention
```
...
```bash
birkin skills uninstall <skill-name>
```
...
| **honcho** | Configure and use Honcho memory with Birkin — cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. |
...
| Skill | Description |
|-------|-------------|
| **blender-mcp** | Control Blender directly from Birkin via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. |
...
| Skill | Description |
|-------|-------------|
| **openclaw-migration** | Migrate a user's OpenClaw customization footprint into Birkin. Imports memories, SOUL.md, command allowlists, user skills, and selected workspace assets. |
...
| **birkin-atropos-environments** | Build, test, and debug Birkin RL environments for Atropos training. Covers the BirkinAgentBaseEnv interface, reward functions, agent loop integration, and evaluation. |
...
| **telephony** | Give Birkin phone capabilities — provision a Twilio number, send/receive SMS/MMS, make calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
