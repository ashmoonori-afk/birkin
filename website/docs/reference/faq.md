...
description: "Frequently asked questions and solutions to common issues with Birkin"
---
...
### What LLM providers work with Birkin?

Birkin works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **Your model provider** — subscription-based inference endpoint
...
Set your provider with `birkin model` or by editing `~/.birkin/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Birkin requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Birkin from inside it. The standard install command works perfectly in WSL2:
...
Yes — Birkin now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Birkin does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.birkin/`.

### Can I use it offline / with local models?

Yes. Run `birkin model`, select **Custom endpoint**, and enter your server's URL:
...
Birkin persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Birkin — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Birkin auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `BIRKIN_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Birkin itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Birkin instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Birkin programmatically:

```python
from run_agent import AIAgent

agent = AIAgent(model="openrouter/nous/birkin-3-llama-3.1-70b")
...
#### `birkin: command not found` after installation
...
**Cause:** Birkin requires Python 3.11 or newer.
...
# Or set directly
birkin config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.birkin/.env` for conflicting entries.
:::
...
# Set a valid model
birkin config set BIRKIN_MODEL openrouter/nous/birkin-3-llama-3.1-70b

# Or specify per-session
birkin chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `birkin chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Birkin detected the wrong context length for your model.
...
birkin chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Birkin may have the wrong context length for your model. Check what it detected:
...
**Cause:** Birkin detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Birkin never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `birkin chat`
...
# Start the gateway
birkin gateway start

# Check logs for errors
cat ~/.birkin/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `birkin gateway setup`
- Check gateway logs: `cat ~/.birkin/logs/gateway.log | tail -50`
...
Configure in `~/.birkin/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "birkin-agent[telegram]"   # or [discord], [slack], [whatsapp]
...
#### WSL: Gateway keeps disconnecting or `birkin gateway start` fails
...
tmux new -s birkin 'birkin gateway run'
# Reattach later: tmux attach -t birkin

# Option 3: Background via nohup
nohup birkin gateway run > ~/.birkin/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'birkin gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `birkin gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
birkin gateway install    # Re-snapshots your current PATH
birkin gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.birkin.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `birkin chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `birkin chat -t "terminal"`
...
cd ~/.birkin/birkin-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.birkin/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
birkin config show | grep -A 12 mcp_servers

# Restart Birkin or reload MCP after config changes
birkin chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Birkin](/docs/guides/use-mcp-with-birkin)
...
:::warning
If an MCP server crashes mid-request, Birkin will report a timeout. Check the server's own logs (not just Birkin logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `birkin profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `birkin update`?

`birkin update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `birkin update` once — it covers every profile on the machine.
...
birkin profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
birkin profile import ./work-backup.tar.gz work
```
...
There is no hard limit. Each profile is just a directory under `~/.birkin/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Birkin can route subagents to a different model automatically. Set this in `~/.birkin/config.yaml`:
...
Now when you tell Birkin "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Birkin do this?

**Current limitation:** Birkin profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Birkin reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `birkin skills config` settings don't seem to take effect.

**Solution:** Use `birkin skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`birkin gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Birkin creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Birkin to another machine
...
1. Install Birkin on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/MoonGwanghoon/birkin/main/scripts/install.sh | bash
   ```

2. Copy your entire `~/.birkin/` directory **except** the `birkin-agent` subdirectory (that's the code repo — the new install has its own):
   ```bash
   # On the source machine
   rsync -av --exclude='birkin-agent' ~/.birkin/ newmachine:~/.birkin/
   ```

   Or use profile export/import:
   ```bash
   # On source machine
   birkin profile export default ./birkin-backup.tar.gz

   # On target machine
   birkin profile import ./birkin-backup.tar.gz default
   ```

3. On the new machine, run `birkin setup` to verify API keys and provider config are working. Re-authenticate any messaging platforms (especially WhatsApp, which uses QR pairing).

The `~/.birkin/` directory contains everything: `config.yaml`, `.env`, `SOUL.md`, `memories/`, `skills/`, `state.db` (sessions), `cron/`, and any custom plugins. The code itself lives in `~/.birkin/birkin-agent/` and is installed fresh.

### Permission denied when reloading shell after install

**Scenario:** After running the Birkin installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Birkin-specific issue — it's a shell config permissions problem.
...
birkin config show | head -20

# Re-run model selection
birkin model

# Or test with a known-good model
birkin chat -q "hello" --model anthropic/claude-sonnet-4.6
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/MoonGwanghoon/birkin/issues)
2. **File a bug report:** Include your OS, Python version (`python3 --version`), Birkin version (`birkin --version`), and the full error message
