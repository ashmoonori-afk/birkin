# OpenAI-Compatible API Server for Birkin
...
SSE streaming. By exposing this endpoint, birkin-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  birkin-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.birkin/.env`
2. Run `birkin gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with birkin-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns birkin-agent as a model) |
...
Authorization: Bearer birkin-api-key-here
...
But birkin-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "birkin-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured birkin-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of birkin-agent's core prompt
...
Once implemented, birkin-agent works as a drop-in backend for:
