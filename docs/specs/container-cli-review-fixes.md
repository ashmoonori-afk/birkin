...
**PR:** NousResearch/birkin-agent#7543
...
1. **Let it crash.** No silent fallbacks. If `.container-mode` exists but something goes wrong, the error propagates naturally (Python traceback). The only case where container routing is skipped is when `.container-mode` doesn't exist or `BIRKIN_DEV=1`.
...
1. get_container_exec_info()
   - BIRKIN_DEV=1 → return None (skip routing)
...
2. _exec_in_container(container_info, sys.argv[1:])
   ...
   c. Build exec_cmd: [sudo? + runtime, "exec", tty_flags, "-u", exec_user, env_flags, container, birkin_bin, *cli_args]
...
### Changes to `birkin_cli/config.py`
...
Change: Remove the try/except around file reading. Keep the early returns for `BIRKIN_DEV=1` and `_is_inside_container()`. The `FileNotFoundError` from `open()` when `.container-mode` doesn't exist should still return `None` (this is the "container mode not enabled" case). All other exceptions propagate.
...
Note: there is a theoretical race between the `[ -d ... ]` check and the `mv` (something could create/remove the directory in between). In practice this is a NixOS activation script running as root during `nixos-rebuild switch` — no other process should be touching `~/.birkin` at that moment. Not worth adding locking for.
...
The existing test file (`tests/birkin_cli/test_container_aware_cli.py`) has 16 tests. With the simplified exec model, several are obsolete.
...
- `test_get_container_exec_info_skipped_when_birkin_dev` — unchanged
- `test_get_container_exec_info_not_skipped_when_birkin_dev_zero` — unchanged
...
- Changes to the `BIRKIN_DEV=1` bypass
