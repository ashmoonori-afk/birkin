...
## 프로덕션 이식 (2026-07-25 완료)

`mnemosyne.py`가 벤치마크 수치를 문서에 인용하면서 정작 무가중 BM25(k1=1.5,
b=0.75)를 출하하던 격차를 닫았다. 동일 하네스(LongMemEval-S cleaned, 470문항)
재측정:

| 구성 | R@1 | R@5 | MRR |
|---|---|---|---|
| 이식 전 (k1=1.5/0.75, 질의 무가중) | 0.870 | 0.968 | 0.910 |
| + idf¹ 질의 가중 | 0.889 | 0.970 | 0.924 |
| **+ k1=0.9, b=0.5 (현행 프로덕션)** | **0.891** | **0.974** | **0.926** |

회귀 게이트(한국어 exact/partial/mixed 스위트, 120노트 한국어 볼트 픽스처
title 조회): 이식 전후 **완전 동일** — 1.0/1.0/0.938, R@1 0.867. 한국어 회귀
없음.

시간 사전확률: `temporal_target()`이 어제/그저께/지난주/지난달/작년·`N일|주|
개월|년 전`을 영어 형태와 함께 파싱하고, 단서가 있을 때만 lam=0.3 가우시안을
곱한다(단서 없으면 점수 불변). 단서 없는 질의는 비트 단위로 동일. **주의**:
lam=0.3은 LongMemEval 세션 날짜에서 튜닝된 값을 노트 updated/created에 그대로
적용한 것이며, 볼트에서 독립 측정한 값이 아니다.

**미채택 — BM25F 필드 가중(title/tags > body).** 구현해서 인덱스에 정상
반영되는 것까지 확인했으나(title 항 5×), 오프라인에서 접근 가능한 어떤
코퍼스도 이득·손해를 판별하지 못했다: 한국어 픽스처는 title/cue 프로브가
1.0으로 포화, half-title 프로브는 가중치 2/3/5 전부에서 R@1 0.05로 불변,
실제 birkin 볼트는 19노트로 너무 작다. INDEX_VERSION 범프(전 사용자 인덱스
재구축)를 요구하는 변경을 미측정 상태로 넣지 않고 되돌렸다. 판별 가능한
코퍼스가 생기면(=아래 L2 annotate + 실vault paraphrase 세트) 재평가한다.

후속 (미착수): real-vault에서 L2 앵커 증류(annotate) 실험.
