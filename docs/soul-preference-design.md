...
4. 캡처는 프롬프트 힌트가 아니라 **대화 후 백그라운드 리뷰**로 끌어올린다.
   (단 "보장"은 과한 표현이다 — §9.6 참조.)
...
```jsonc
"profile": {
  "enabled": false,        // 초기 롤아웃은 다크(§9.4). 비활성 시 디스크에 아무것도
                           // 만들지 않는다 — 스냅샷 읽기가 부트스트랩을 유발하면 안 됨.
...
### 9.6 구현 중 확정된 사실 (설계 정정)

구현·QA를 거치며 설계가 틀렸던 지점과, 이후 유지해야 할 불변식이다.

**캡처는 보장이 아니라 best-effort다.** 배경 리뷰는 인메모리 큐 위에서 돌기 때문에
프로세스가 급종료되면 대기 중인 교환은 사라진다. durable outbox를 넣기 전까지
"보장"이라고 쓰지 않는다. `profile_review.py` 독스트링과 README가 이를 명시한다.

**읽기는 절대 쓰기를 유발하지 않는다.** 초기 구현은 `snapshot()`이 파일이 없으면
`bootstrap()`을 호출했고, `runtime`이 `enabled` 검사 없이 스냅샷을 읽어서
**비활성 상태인데도 `<BIRKIN_HOME>/profile/`가 생성됐다.** 다크 롤아웃의 의미가
사라지는 결함이었다. 이후 `snapshot()`은 디렉터리가 없으면 빈 스냅샷을 반환하고
아무것도 만들지 않으며, 부트스트랩은 활성 경로에서 명시적으로만 호출한다.

**설정 검증은 스키마 정규화보다 먼저 돌아야 한다.** `config.py`에
`profile.enabled must be boolean`을 던지는 검증기가 있었지만 `config_model.py`의
스키마 워커가 먼저 잘못된 값을 기본값으로 치환해 **검증기가 도달 불가능한 죽은 코드**였다.
`{"profile": {"enabled": "yes"}}` 같은 흔한 오타에서 기능이 조용히 꺼졌다.
이제 raw `profile` 서브트리를 정규화 전에 검증한다. 기능 게이트에 한해 하우스의
warn-and-default 관례보다 loud failure를 택한 것이며, 누락된 키는 여전히 기본값과 병합된다.

**마이그레이션 재실행은 진짜 no-op이어야 한다.** 항목 중복이 없는 것만으로는 부족하다 —
바뀐 것이 없으면 매니페스트 파일 자체를 쓰지 않는다(해시 불변으로 검증). 리포트는
`unchanged` 필드로 "이번에 한 일 없음"과 "방금 처리함"을 구분한다. 미완료 마이그레이션의
재개 경로는 그대로 유지된다.
