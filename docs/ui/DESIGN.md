...
프레임 순서는 모든 폭에서 동일하다: 헤더 1줄 · 구분선 · 대화 · 구분선 · 탭 · 활성 패널 · composer · 키 힌트.
헤더는 `Birkin · <활성 패널>` 과 `<connection> · ledger <cursor>` 를 한 줄에 담고, ledger 수치는 snapshot cursor에서 파생한다(장식 라벨 금지).

| 레이아웃 | 조건 | 밀도 |
| --- | --- | --- |
| Wide | ≥120 | 탭 전량 노출, 상시 키 힌트 |
| Medium | 80–119 | 탭은 넘치는 개수만 `+N`으로 접음. 워크벤치 Ledger 레일은 80–99에서 glyph(4열)로 압축 |
| Narrow | <80 | 헤더 우측(connection·ledger)은 자리가 없으면 생략, 힌트는 `Tab · ↑/↓ · Enter · Esc`로 축약 |

rows 예산은 크롬(헤더·구분선·탭·패널·composer·힌트) 우선으로 확보하고 남는 줄에 최신 대화를 채운다 — 화면이 짧아도 헤더와 composer는 사라지지 않는다.
검증 크기: 60×20, 80×24, 120×30, 160×40. 폭이 좁다고 대화를 패널 뒤로 밀지 않는다 — 채팅은 모든 폭에서 1순위다.
