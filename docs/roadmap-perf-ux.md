...
P0~P2 전 항목 순차 구현·페이즈별 검증 완료. 페이즈별 게이트(전체 스위트)
통과, 항목별 회귀 테스트 고정.

| 항목 | 상태 | 커밋 |
|---|---|---|
| P0-1 편집-스트리밍 하드닝 | ✅ | a82778f |
| P0-2 인라인버튼 승인 인박스 | ✅ | 5b03676 |
| P0-3 Morpheus 아침 다이제스트 | ✅ | 30cf6ab |
| P1-1 REPL 패리티(OpenAI SSE·마크다운·웜세션) | ✅ | f7e8d34, c1a6c98 |
| P1-2 메모리 투명성(REPL) | ✅ (Telegram은 후속) | 6a018e0 |
| P1-3 침묵 제거(친화 에러·백오프 표시) | ✅ | 749b6be |
| P1-4 온보딩(welcome·무료경로·oauth 테스트) | ✅ | dbc3930 |
| P2-1 멀티모달 수신(이미지·파일; STT 후속) | ✅ | d19abd1 |
| P2-2 프롬프트 캐싱 | ✅ 이미 구현됨(회귀 테스트 추가) | 92d7fc3 |
| P2-3 /remind 채팅 스케줄 | ✅ | 0de311b |

**정직한 미구현/후속** (근거와 함께):
- P1-2 Telegram 메모리 투명성: `on_text`가 응답-텍스트 전용 계약이라 tool
  상태를 섞으면 스트리머 finalize가 깨짐 → 별도 on_event 채널 필요.
- P2-1 STT/TTS/이미지생성: 전부 외부 API 필요(zero-dep 위반) → 사용자 결정
  대기. 수신·저장·경로전달(이미지가 최대 가치)만 출고.
- P3 (Leon 3-scope, 상시 하트비트): 벤치/이득 미검증으로 보류.
