...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.birkin/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("BIRKIN_HOME", Path.home() / ".birkin")) / "skills" / "red-teaming" / "godmode"
...
BIRKIN_HOME = Path(os.getenv("BIRKIN_HOME", Path.home() / ".birkin"))
CONFIG_PATH = BIRKIN_HOME / "config.yaml"
PREFILL_PATH = BIRKIN_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Birkin models — already uncensored, just needs clean prompt
    "birkin": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "birkin" in model_lower or "nous" in model_lower:
        return "birkin"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Birkin config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.birkin/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Birkin for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Birkin for changes to take effect.")
