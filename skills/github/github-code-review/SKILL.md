...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.birkin/.env ] && grep -q "^GITHUB_TOKEN=" ~/.birkin/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.birkin/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source ~/.birkin/skills/github/github-auth/scripts/gh-env.sh
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Birkin. Code looks clean — good test coverage, no security concerns."
