...
- Give it a name like "birkin-agent"
- Select scopes:
...
- Give it a title like "birkin-agent-<machine-name>"
...
elif [ -f ~/.birkin/.env ] && grep -q "^GITHUB_TOKEN=" ~/.birkin/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.birkin/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
