"""
BirkinAgentBaseEnv -- Abstract Base Environment for Birkin-Agent + Atropos

Provides the Atropos integration plumbing that all birkin-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via BirkinAgentLoop
...
# Ensure the birkin-agent repo root is on sys.path so that imports like
...
# Load API keys from birkin-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, BirkinAgentLoop
...
# Import birkin-agent toolset infrastructure
...
class BirkinAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for birkin-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of birkin toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: birkin, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/birkin-results/ "
...
class BirkinAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for birkin-agent Atropos integration.
    ...
    name: Optional[str] = "birkin-agent"
    env_config_cls = BirkinAgentEnvConfig

    def __init__(
        self,
        config: BirkinAgentEnvConfig,
        ...
        # Set terminal environment variables so birkin tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY birkin-agent tool (terminal, file, web,
