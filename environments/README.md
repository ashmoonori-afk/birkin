# Birkin-Agent Atropos Environments

This directory contains the integration layer between **birkin-agent's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  BirkinAgentBaseEnv    │  birkin_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     BirkinSweEnv    TerminalBench2EvalEnv
...
**BirkinAgentBaseEnv** (`birkin_base_env.py`) extends BaseEnv with birkin-agent specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves birkin-agent toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `BirkinAgentBaseEnv` and implement:
...
`BirkinAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as birkin-agent's `run_agent.py`:
...
- **Generic**: `call_tool(name, args)` -- call any birkin-agent tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some birkin-agent tools use `asyncio.run()` internally (e.g., the Modal backend). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Available parsers:
- `birkin` -- Birkin/ChatML `<tool_call>` XML format
...
├── birkin_base_env.py            # Abstract base (BirkinAgentBaseEnv)
...
│
├── birkin_swe_env/               # SWE-bench style training environment
│   └── birkin_swe_env.py
│
...
2. Create your env file inheriting from `BirkinAgentBaseEnv`
...
2. Inherit from `BirkinAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which birkin toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `birkin` |
