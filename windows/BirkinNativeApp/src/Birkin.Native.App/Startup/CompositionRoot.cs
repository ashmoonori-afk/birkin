...
public sealed class CompositionRoot : IAsyncDisposable
{
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        await ShutdownAsync(
            Runner.StopObservingOwnedProcessAsync,
            () => Supervisor.ShutdownAsync(
                () => Session.SendGoodbyeAsync(CancellationToken.None),
                Coordinator.DisposeAsync)).ConfigureAwait(false);
    }

    internal static async ValueTask ShutdownAsync(
        Func<ValueTask> stopObserving,
        Func<ValueTask> shutdown)
    {
        try
        {
            await stopObserving().ConfigureAwait(false);
        }
        finally
        {
