        ...
        await using (bridge)
        {
            ...
            var journey = await sta.InvokeAsync(async () =>
            {
                ...
                try
                {
                    ...
                    try
                    {
                        stage = "office-command";
                        var submitted = await composition.Coordinator.CreateOfficeDocumentAsync(
                            new OfficeCreateIntent(
                                "docx",
                                new OfficeDocumentContent(["W4_DETERMINISTIC_WINDOW_SENTINEL"]),
                                "w4-deterministic-window.docx"),
                            deadline.Token);
                        Assert.IsTrue(submitted, "real Python authority refused office.create");
