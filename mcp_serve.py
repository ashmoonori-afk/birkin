"""
Birkin MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Birkin-specific extra)

Usage:
    birkin mcp serve
    birkin mcp serve --verbose
...
logger = logging.getLogger("birkin.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using BIRKIN_HOME."""
    try:
        from birkin_constants import get_birkin_home
        return get_birkin_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("BIRKIN_HOME", Path.home() / ".birkin")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from birkin_constants import get_birkin_home
        directory_file = get_birkin_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("BIRKIN_HOME", Path.home() / ".birkin")
...
class EventBridge:
    ...
    This is the Birkin equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from birkin_constants import get_birkin_home
            db_file = get_birkin_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("BIRKIN_HOME", Path.home() / ".birkin")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Birkin MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'birkin-agent[mcp]'"
        )

    mcp = FastMCP(
        "birkin",
        instructions=(
            "Birkin messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Birkin MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
