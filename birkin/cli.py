    ...
    try:
        ...
        if args.action == "resolve":
            ...
            print(
                ...
                if args.json
                else f"{found.name}@{found.version} [{found.scope.value}] {found.path}"
            )
            return 0
        ...
        print(
            ...
            if args.json
            else f"Installed {installed.name}@{installed.version} [{installed.scope.value}]"
        )
        return 0
    except (
        ManifestError,
...
def _skills_sync(args: argparse.Namespace) -> int:
    ...
    if not sources:
        print(
            "No source found. Pass --from <skills-dir> (e.g. a hermes skills "
            "directory)."
        )
        return 1
    total = 0
...
def _skills_install(args) -> int:
    ...
    if not identifier:
        print(
            "Which skill? `birkin skills install <owner/repo[/path] | "
...
def _skills_audit(args) -> int:
    ...
    if lock:
        ...
        for name, entry in sorted(lock.items()):
            print(
                f"  {name:<24} {entry.get('identifier', '?'):<36} "
...
def _cmd_omo(_args: argparse.Namespace) -> int:
    from .gateway import core
    from .gateway.telegram_lease import format_gateway_diagnostics

    print(format_gateway_diagnostics(core.config.load_config()))
    return 0
...
def _cmd_harness(args: argparse.Namespace) -> int:
    ...
    target = (
        ...
        else (args.target[0] if args.target else "")
    )
    ...
    if args.action == "show":
        ...
        print(
            block
            or f"The {scope} harness is empty — nothing recorded yet. "
    ...
    try:
        artifact, path = harness.record_refine_request(
            target,
            scope=scope,
        ...
        print(f"could not persist refine request: {exc}", file=sys.stderr)
        return 1
    print(f"recorded refine request {artifact['id']} at {path}")
    return 0
    ...
    try:
        ...
        def snapshot_locked() -> dict[str, object]:
            ...
            return {
                ...
                **{field: list(working[field]) for field in harness.WORKING_FIELDS},
                "updated_at": (
                    working["updated_at"]
        ...
        if args.goal is None and not any(updates.values()):
        ...
        print(
            f"Updated working memory for {session_id} (revision {state['revision']})."
        )
        return 0
...
def _cmd_curate(args: argparse.Namespace) -> int:
    ...
    report = curator.curate(build_manager(config.load_config()), apply=not args.dry_run)
...
def _cmd_reindex(args: argparse.Namespace) -> int:
    ...
    print(
        f"reindexed: {stats['notes']} notes, {stats['zones']} zones, "
...
def _cmd_tools(args: argparse.Namespace) -> int:
    ...
    ctx = ToolContext(
        cfg=base, client=client, cwd=Path.cwd(), skills=skills, memory=memory
    )
...
def _cmd_computer_use(args: argparse.Namespace) -> int:
    ...
    if args.computer_use_action == "setup":
        ...
        service = create_service(
            ...
            policy_config=(
                computer_use_policy if isinstance(computer_use_policy, dict) else None
            ),
        )
...
def _cmd_permission(args: argparse.Namespace) -> int:
    ...
    if args.access in ("workspace", "full"):
        ...
        idx = menu.select(
            "CLI-agent access level (Claude Code / Codex)", labels, default=di
        )
        if idx is not None:
            chosen = _CLI_ACCESS_LEVELS[idx][0]
            if chosen == "full":
                print(
                    "⚠  'full' lets the CLI agent run ANY command and edit ANY "
    ...
    print(
        "  "
        + (", ".join(cfg.get("auto_approve", [])) or "(none)")
        + "   (everything else is queued for `birkin review`)"
    )
    print(
        f"CLI-agent access level: {cfg.get('cli_access', 'workspace')}"
...
def _cmd_moirai(args: argparse.Namespace) -> int:
    ...
    handlers = {
        "run": moirai_cli.cmd_run,
        "list": moirai_cli.cmd_list,
...
def _auth_claude(action: str) -> int:
    ...
    if "access_token_len" in info:
        print(
            f"  tokens  : access={info['access_token_len']}자, "
...
def _cmd_sessions(args: argparse.Namespace) -> int:
    ...
    export_all = bool(getattr(args, "all", False) or getattr(args, "legacy_all", False))
    to_vault = bool(
        getattr(args, "vault", False) or getattr(args, "legacy_vault", False)
    )
    stems = (
        [f.stem for f in sessions_export.list_sessions()]
        if export_all
        else [s for s in (args.name or []) if s != "--"]
    )
    if not stems:
...
def _cmd_odyssey(args: argparse.Namespace) -> int:
    ...
    print(
        "Run `/odyssey` inside `birkin` (or message your gateway) to drive the cycle."
    )
    return 0
...
def _cmd_neurosis(args: argparse.Namespace) -> int:
    ...
    seed = neurosis.seed_or_resume(
        idea, cfg=config.load_config(), resolution=resolution
    )
    if seed is None:
        print('Give an idea, e.g. `birkin neurosis "a tool that ..."`.')
        return 1
    print(
        f"neurosis '{seed['slug']}' seeded · threshold {seed['threshold_percent']} "
        f"(source: {seed['threshold_source']})."
    )
    print(f"  state: {seed['state_path']}")
    print(f"  spec : {seed['spec_path']}")
    print(
        "Run `/neurosis` inside `birkin` (or message your gateway) to drive the "
...
def _cmd_daedalus(args: argparse.Namespace) -> int:
    ...
    try:
        if action == "create":
            ...
            print(
                f"daedalus '{doc['slug']}' created at {doc['token']} "
                f"({len(doc['nodes'])} nodes)"
            )
        elif action == "refresh":
            doc = daedalus.refresh(
                args.slug, root, expected_token=args.expected_token, cfg=cfg
            )
            print(
                f"daedalus '{doc['slug']}' refreshed to {doc['token']} "
                f"({len(doc['nodes'])} nodes)"
            )
        elif action == "note":
            doc = daedalus.add_note(
                args.slug, args.text, refs=tuple(args.ref or ()), cfg=cfg
            )
            ...
            if doc is None:
                print(
                    f"error: no document '{args.slug}'; run "
...
def _run_companion(companion, args: argparse.Namespace) -> int:
    ...
    if action == "bind":
        ...
        print(
            f"bound {record['id']} (check-ins stay off until "
    ...
    if action == "policy":
        ...
        if args.quiet_start and args.quiet_end:
            changes["quiet_hours"] = {"start": args.quiet_start, "end": args.quiet_end}
        if args.daily_cap is not None:
            changes["daily_cap"] = args.daily_cap
        if args.cooldown is not None:
            changes["cooldown_minutes"] = args.cooldown
        policy = companion.set_policy(**changes) if changes else companion.get_policy()
...
def _cmd_cron(args: argparse.Namespace) -> int:
    ...
    for j in jobs:
        ...
        print(
            f"{j['id']}  {cron.schedule_display(j)}  "
...
def _cmd_runs(args: argparse.Namespace) -> int:
    ...
    for r in runs:
        ...
        print(
            f"{DIM}{when}{RESET}  {CYAN}{r.get('kind'):7}{RESET} ~{tok:>5} tok  "
...
def _cmd_budget(args: argparse.Namespace) -> int:
    ...
    fits = [
        name
        for name, cap in sorted(tiers.items(), key=lambda kv: kv[1])
        if month_usd <= cap
    ]
    verdict = (
        f"fits the {fits[0]} credit tier (${tiers[fits[0]]}/mo)"
        if fits
        else "exceeds every announced credit tier"
    )
...
def _cmd_trace(args: argparse.Namespace) -> int:
    ...
    for path in matches[:5]:
        ...
        print(
            f"{BOLD}── {rec.get('id')}{RESET}  {DIM}{rec.get('at', '')}{RESET}  "
        ...
        if usage:
            print(
                f"  usage  : ~{usage.get('estTokens', 0)} tok "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    handlers = {
        "_cmd_auth": _cmd_auth,
        "_cmd_budget": _cmd_budget,
        "_cmd_chat": _cmd_chat,
        "_cmd_companion": _cmd_companion,
        "_cmd_compare": _cmd_compare,
        "_cmd_computer_use": _cmd_computer_use,
        "_cmd_cron": _cmd_cron,
        "_cmd_curate": _cmd_curate,
        "_cmd_curate_memory": _cmd_curate_memory,
        "_cmd_daedalus": _cmd_daedalus,
        "_cmd_daemon": _cmd_daemon,
        "_cmd_gateway": _cmd_gateway,
        "_cmd_harness": _cmd_harness,
        "_cmd_lineage": _cmd_lineage,
        "_cmd_live_sessions": _cmd_live_sessions,
        "_cmd_mcp": _cmd_mcp,
        "_cmd_mcp_serve": _cmd_mcp_serve,
        "_cmd_model": _cmd_model,
        "_cmd_moirai": _cmd_moirai,
        "_cmd_morpheus": _cmd_morpheus,
        "_cmd_native_bridge": _cmd_native_bridge,
        "_cmd_native_provider_probe": _cmd_native_provider_probe,
        "_cmd_neurosis": _cmd_neurosis,
        "_cmd_odyssey": _cmd_odyssey,
        "_cmd_omo": _cmd_omo,
        "_cmd_permission": _cmd_permission,
        "_cmd_plugin_effects": _cmd_plugin_effects,
        "_cmd_plugins": _cmd_plugins,
        "_cmd_reindex": _cmd_reindex,
        ...
        "_cmd_voice": _cmd_voice,
        "_cmd_web": _cmd_web,
        "_cmd_worker_hook_qa": _cmd_worker_hook_qa,
        "_cmd_working_memory": _cmd_working_memory,
    }
    return build_cli_parser(version=__version__, handlers=handlers)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if command_line and command_line[0] == "_approval-helper":
