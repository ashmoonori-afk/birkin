        ...
        while True:
            ...
            for event in events:
                ...
                if event.type == "message.assistant.delta":
                    ...
                    event_name = (
                        runtime_event if isinstance(runtime_event, str) else event.type
                    )
...
def _panels(session: Session) -> list[dict[str, object]]:
    ...
    return [{"key": key, "label": key, "items": sources[key]} for key in PANEL_KEYS]
...
def run(
    ...
    try:
        ...
        try:
            shared_session, runtime_session = web_server.workspace_runtime(session_id)
        except ConfigError as exc:
            print(f"{ui.RED}{exc}{ui.RESET}")
            return 1
        runtime_session.workspace_snapshot = lambda: web_server.workspace_snapshot(
            session_id
        )
