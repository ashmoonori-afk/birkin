...
Development commits never increment the package version automatically. The tracked `scripts/hooks/pre-commit` is intentionally non-mutating. For a release, update `pyproject.toml` and `birkin/__init__.py` explicitly, then push the matching `vX.Y.Z` tag; the release workflow rejects a mismatched tag. If an older mutating hook is already installed in `.git/hooks/pre-commit`, replace it with the tracked hook or remove it.
