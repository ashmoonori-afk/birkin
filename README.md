...
| Mandatory runtime dependencies | 1 (voice SDK) | Large exact-pinned Python set plus extras | Multiple npm package dependency graphs; Python runtime uses IPython |
...
**Small dependency and supply-chain surface.** The only mandatory runtime
dependency is the OpenAI SDK used by voice. HTTP, streaming, JSON-RPC, cron
parsing, persistence, provider clients, and the native agent loop are
implemented in the package. Optional desktop screenshots use Pillow and pywin32
