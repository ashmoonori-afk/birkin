...
it is automatically downloaded from GitHub releases to $BIRKIN_HOME/bin/tirith.
...
def _get_birkin_home() -> str:
    """Return the Birkin home directory, respecting BIRKIN_HOME env var."""
    return str(get_birkin_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_birkin_home(), ".tirith-install-failed")
...
def _birkin_bin_dir() -> str:
    """Return $BIRKIN_HOME/bin, creating it if needed."""
    d = os.path.join(_get_birkin_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $BIRKIN_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_birkin_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $BIRKIN_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $BIRKIN_HOME/bin/tirith
    ...
    birkin_bin = os.path.join(_birkin_bin_dir(), "tirith")
    if os.path.isfile(birkin_bin) and os.access(birkin_bin, os.X_OK):
        _resolved_path = birkin_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return birkin_bin
