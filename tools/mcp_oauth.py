...
This module provides the glue:
    - ``BirkinTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Birkin"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses BIRKIN_HOME so each profile gets its own OAuth tokens.
    Layout: ``BIRKIN_HOME/mcp-tokens/``
    """
    try:
        from birkin_constants import get_birkin_home
        base = Path(get_birkin_home())
    except ImportError:
        base = Path(os.environ.get("BIRKIN_HOME", str(Path.home() / ".birkin")))
...
# ---------------------------------------------------------------------------
# BirkinTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class BirkinTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        BIRKIN_HOME/mcp-tokens/<server_name>.json         -- tokens
        BIRKIN_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Birkin.</p></body></html>"
            ) if code else (
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = BirkinTokenStorage(server_name)
    storage.remove()
...
def build_oauth_auth(
    ...
    # --- Client metadata ---
    client_name = cfg.get("client_name", "Birkin")
    scope = cfg.get("scope")
