...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.birkin/.env``.
"""
...
def _managed_persistence_enabled() -> bool:
    """Return whether Birkin-managed persistence is enabled for Camofox.
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the Birkin profile so the Camofox server can map it
    ...
    with _sessions_lock:
        ...
        if _managed_persistence_enabled():
            ...
            session = {
                "user_id": f"birkin_{uuid.uuid4().hex[:10]}",
                "tab_id": None,
...
def camofox_vision(question: str, annotate: bool = False,
    ...
    try:
        ...
        screenshots_dir = get_birkin_home() / "browser_screenshots"
