...
from cli import BirkinCLI, _build_compact_banner, _rich_text_from_ansi
from birkin_cli.skin_engine import get_active_skin, set_active_skin


def _make_cli_stub():
    cli = BirkinCLI.__new__(BirkinCLI)
...
class TestCliSkinPromptIntegration:
    ...
    def test_icon_only_skin_symbol_still_visible_in_special_states(self):
        ...
        with patch("birkin_cli.skin_engine.get_active_prompt_symbol", return_value="⚔ "):
            assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "🔑 ⚔ ")]
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_nous_birkin_branding(self):
        set_active_skin("default")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch("cli.format_banner_version_label", return_value="Birkin v0.1.0 (test)"):
            banner = _build_compact_banner()

        assert "NOUS BIRKIN" in banner

    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_birkin(self):
        ...
        assert "NOUS BIRKIN" not in banner
    ...
    def test_compact_banner_shows_version_label(self):
        ...
        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch("cli.format_banner_version_label", return_value="Birkin v1.0 (test) · upstream abc12345"):
            banner = _build_compact_banner()
