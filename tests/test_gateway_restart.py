...
def test_hard_restart_records_origin_and_writes_greeting_marker(tmp_path, monkeypatch):
    ...
    marker = core._restart_marker_path()
    monkeypatch.setattr(core, "_restart_marker_path", lambda: marker)
    monkeypatch.setattr(os, "name", "posix")
    ...
    assert store._read_json(marker, None) == {"channel": "telegram", "chat_id": "555"}
