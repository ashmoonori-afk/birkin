...
def _stub_engine(monkeypatch, phases=("할 일 1/3: 검색",)):
    ...
    monkeypatch.setattr(
        engine, "load_script", lambda path: types.SimpleNamespace(name="hard-task")
    )
...
class TestTheExecutorForwardsEvents:
    ...
    def test_execute_claimed_forwards(self, monkeypatch, tmp_path) -> None:
        ...
        events: list[str] = []
        out = approval_execution.execute_claimed(
            rec["id"],
            approvals.execute_action,
            on_event=lambda e, p: events.append(e),
        )
...
class TestGatewayMapsPhaseIntoProgress:
    def test_a_phase_event_lands_in_the_holder(self, monkeypatch, tmp_path) -> None:
        monkeypatch.setenv("BIRKIN_HOME", str(tmp_path))
    ...
    def test_without_a_callback_nothing_changes(self, monkeypatch, tmp_path) -> None:
        monkeypatch.setenv("BIRKIN_HOME", str(tmp_path))
...
class TestTelegramWrapsApprovalExecution:
    ...
    def test_a_progress_capable_gateway_feeds_the_holder(self, monkeypatch) -> None:
        ch = self._channel(monkeypatch)
        out = tg.execute_claimed_with_progress(self._GatewayNew(), ch, "42", "aid")
        assert out == "ok-new"
    ...
    def test_an_old_gateway_is_not_broken(self, monkeypatch) -> None:
        ch = self._channel(monkeypatch)
        assert (
            tg.execute_claimed_with_progress(self._GatewayOld(), ch, "42", "aid")
            == "ok-old"
        )
...
class TestHeartbeatRendersThePhase:
    def test_the_phase_appears_in_the_line(self) -> None:
        line = tg.heartbeat_text(3, {"phase": "할 일 2/5: 자료 정리", "activity": 2})
        assert "할 일 2/5" in line

    def test_no_phase_reads_like_before(self) -> None:
        line = tg.heartbeat_text(
            3, {"activity": 2, "streamed": 0, "last_kind": "command_execution"}
        )
