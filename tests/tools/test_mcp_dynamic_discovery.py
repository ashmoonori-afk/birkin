...
class TestRegisterServerTools:
    ...
    def test_injects_birkin_toolsets(self, mock_registry, mock_toolsets):
        """Tools are injected into birkin-* toolsets but not custom ones."""
        ...
        # Injected into birkin-* toolsets
        assert "mcp_my_srv_my_tool" in mock_toolsets["birkin-cli"]["tools"]
        assert "mcp_my_srv_my_tool" in mock_toolsets["birkin-telegram"]["tools"]
        # NOT into non-birkin toolsets
...
class TestRefreshTools:
    ...
    async def test_nuke_and_repave(self, mock_registry, mock_toolsets):
        ...
        mock_toolsets["birkin-cli"]["tools"].append("mcp_live_srv_old_tool")
        ...
        assert "mcp_live_srv_old_tool" not in mock_toolsets["birkin-cli"]["tools"]

        # New tool registered
        assert "mcp_live_srv_new_tool" in mock_registry.get_all_tool_names()
        assert "mcp_live_srv_new_tool" in mock_toolsets["birkin-cli"]["tools"]
