...
def _clean_state():
    ...
    saved = {}
    for k in ("BIRKIN_INTERACTIVE", "BIRKIN_GATEWAY_SESSION", "BIRKIN_EXEC_ASK", "BIRKIN_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["BIRKIN_INTERACTIVE"] = "1"
        session_key = os.getenv("BIRKIN_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No BIRKIN_INTERACTIVE or BIRKIN_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
