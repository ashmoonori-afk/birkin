...
def _restore_tool_modules():
    original_birkin_home = os.environ.get("BIRKIN_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_birkin_home is None:
            os.environ.pop("BIRKIN_HOME", None)
        else:
            os.environ["BIRKIN_HOME"] = original_birkin_home
        _reset_modules(("tools", "birkin_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    birkin_cli = types.ModuleType("birkin_cli")
    birkin_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["birkin_cli"] = birkin_cli
    birkin_home = tmp_path / "birkin-home"
    os.environ["BIRKIN_HOME"] = str(birkin_home)
    sys.modules["birkin_cli.config"] = types.SimpleNamespace(
        get_birkin_home=lambda: birkin_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="birkin-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": birkin_home / "modal_snapshots.json",
