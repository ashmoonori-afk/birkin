...
class TestGetBundledDir:
    def test_env_var_override(self, tmp_path, monkeypatch):
        """BIRKIN_BUNDLED_SKILLS env var overrides the default path resolution."""
        custom_dir = tmp_path / "custom_skills"
        custom_dir.mkdir()
        monkeypatch.setenv("BIRKIN_BUNDLED_SKILLS", str(custom_dir))
    ...
    def test_default_without_env_var(self, monkeypatch):
        ...
        monkeypatch.delenv("BIRKIN_BUNDLED_SKILLS", raising=False)
        result = _get_bundled_dir()
        assert result.name == "skills"

    def test_env_var_empty_string_ignored(self, monkeypatch):
        """Empty BIRKIN_BUNDLED_SKILLS should fall back to default."""
        monkeypatch.setenv("BIRKIN_BUNDLED_SKILLS", "")
        result = _get_bundled_dir()
        assert result.name == "skills"
