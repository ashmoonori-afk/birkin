"""Tests for the update check mechanism in birkin_cli.banner."""
...
def test_version_string_no_v_prefix():
    ...
    from birkin_cli import __version__
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "birkin-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("birkin_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BIRKIN_HOME", str(tmp_path))
    with patch("birkin_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import birkin_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "birkin_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when BIRKIN_HOME has no git repo."""
    import birkin_cli.banner as banner
    ...
    # Point BIRKIN_HOME at a temp dir with no birkin-agent/.git
    monkeypatch.setenv("BIRKIN_HOME", str(tmp_path))
    with patch("birkin_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from birkin_cli.main import _invalidate_update_cache

    # Build a fake ~/.birkin with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"BIRKIN_HOME": str(default_home)}):
