...
BIRKIN_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot") while
_PROVIDER_MODELS and config.yaml use Birkin IDs ("copilot").  The slug
...
def test_copilot_uses_birkin_slug():
...
def test_kimi_for_coding_alias():
    ...
    from birkin_cli.auth import resolve_provider
...
def test_kimi_for_coding_overlay_uses_birkin_slug():
...
def test_kilo_overlay_uses_birkin_slug():
