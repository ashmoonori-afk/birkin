"""Tests for Codex auth — tokens stored in Birkin auth store (~/.birkin/auth.json)."""
...
def _setup_birkin_auth(birkin_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Birkin auth store."""
    ...
    auth_file = birkin_home / "auth.json"
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_birkin_auth(birkin_home)
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    birkin_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (birkin_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("BIRKIN_HOME", str(birkin_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    birkin_home = tmp_path / "birkin"
    _setup_birkin_auth(birkin_home, access_token="")
    monkeypatch.setenv("BIRKIN_HOME", str(birkin_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_birkin_auth(birkin_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("BIRKIN_HOME", str(birkin_home))
    ...
    monkeypatch.setattr("birkin_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    birkin_home = tmp_path / "birkin"
    _setup_birkin_auth(birkin_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("BIRKIN_HOME", str(birkin_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Birkin auth store, not ~/.codex/."""
    birkin_home = tmp_path / "birkin"
    codex_home = tmp_path / "codex-cli"
    birkin_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "birkin-at", "refresh_token": "birkin-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Birkin store
    assert not (codex_home / "auth.json").exists()

    # Birkin auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "birkin-at"
...
def test_refresh_codex_auth_tokens_writes_back_to_cli(tmp_path, monkeypatch):
    ...
    from birkin_cli.auth import _refresh_codex_auth_tokens
    ...
    monkeypatch.setattr("birkin_cli.auth.refresh_codex_oauth_pure", lambda *a, **kw: {
...
def test_resolve_returns_birkin_auth_store_source(tmp_path, monkeypatch):
    birkin_home = tmp_path / "birkin"
    _setup_birkin_auth(birkin_home)
    monkeypatch.setenv("BIRKIN_HOME", str(birkin_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "birkin-auth-store"
