"""Tests for the top-level `./birkin` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./birkin` should use `birkin_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "birkin"
    called = []

    fake_main_module = types.ModuleType("birkin_cli.main")

    def fake_main():
        called.append("birkin_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "birkin_cli.main", fake_main_module)
