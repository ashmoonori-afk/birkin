...
def container_env(tmp_path, monkeypatch):
    """Set up a fake BIRKIN_HOME with .container-mode file."""
    birkin_home = tmp_path / ".birkin"
    birkin_home.mkdir()
    monkeypatch.setenv("BIRKIN_HOME", str(birkin_home))
    monkeypatch.delenv("BIRKIN_DEV", raising=False)

    container_mode = birkin_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=birkin-agent\n"
        "exec_user=birkin\n"
        "birkin_bin=/data/current-package/bin/birkin\n"
    )
    return birkin_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("birkin_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "birkin-agent"
    assert info["exec_user"] == "birkin"
    assert info["birkin_bin"] == "/data/current-package/bin/birkin"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("birkin_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_birkin_dev(container_env, monkeypatch):
    """Returns None when BIRKIN_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("BIRKIN_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_birkin_dev_zero(container_env, monkeypatch):
    """BIRKIN_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("BIRKIN_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        birkin_home = Path(tmpdir) / ".birkin"
        birkin_home.mkdir()
        (birkin_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("birkin_constants.is_container", return_value=False), \
             patch("birkin_cli.config.get_birkin_home", return_value=birkin_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("BIRKIN_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=birkin-custom\n"
        "exec_user=myuser\n"
        "birkin_bin=/opt/birkin/bin/birkin\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "birkin-custom"
    assert info["exec_user"] == "myuser"
    assert info["birkin_bin"] == "/opt/birkin/bin/birkin"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from birkin_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "birkin"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "birkin-agent" in cmd
    assert "/data/current-package/bin/birkin" in cmd
    assert "chat" in cmd
