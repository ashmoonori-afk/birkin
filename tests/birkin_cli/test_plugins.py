"""Tests for the Birkin plugin system (birkin_cli.plugins)."""
...
from birkin_cli.plugins import (
    ENTRY_POINTS_GROUP,
    VALID_HOOKS,
    LoadedPlugin,
    PluginContext,
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.birkin/plugins/ are discovered."""
        plugins_dir = tmp_path / "birkin_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("BIRKIN_HOME", str(tmp_path / "birkin_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("BIRKIN_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".birkin" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under birkin_plugins.<name>."""
        plugins_dir = tmp_path / "birkin_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("BIRKIN_HOME", str(tmp_path / "birkin_test"))

        # Clean up any prior namespace module
        sys.modules.pop("birkin_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "birkin_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="birkin_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "birkin_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import birkin_cli.plugins as plugins_mod
...
# in PluginContext (birkin_cli/plugins.py).  The tests referenced _plugin_commands,
