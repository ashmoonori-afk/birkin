...
class TestSmsToolset:
    def test_birkin_sms_toolset_exists(self):
        from toolsets import get_toolset

        ts = get_toolset("birkin-sms")
        assert ts is not None
        assert "tools" in ts

    def test_birkin_sms_in_gateway_includes(self):
        from toolsets import get_toolset

        gw = get_toolset("birkin-gateway")
        assert gw is not None
        assert "birkin-sms" in gw["includes"]
