...
def _isolate(tmp_path, monkeypatch):
    """Isolate BIRKIN_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal BirkinCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = BirkinCLI.__new__(BirkinCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["birkin-core"]
        obj.compact = False
