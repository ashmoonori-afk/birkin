...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("birkin/birkin-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned birkin/* and pr-* branches."""

    def test_prunes_orphaned_birkin_branch(self, git_repo):
        """birkin/birkin-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "birkin/birkin-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "birkin/birkin-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "birkin/birkin-deadbeef" in result.stdout
        ...
        assert "birkin/birkin-deadbeef" in orphaned
        ...
        assert "birkin/birkin-deadbeef" not in result.stdout
