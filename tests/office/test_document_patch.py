...
def test_patch_is_copy_first_atomic_and_precondition_bound(tmp_path):
    ...
    with pytest.raises(DocumentError) as caught:
        apply_patch(src,out,ops,expected_source_sha256=digest)
    ...
    result=apply_patch(src,out,ops[:1],expected_source_sha256=digest)
    assert out.exists() and result!=digest and hashlib.sha256(src.read_bytes()).hexdigest()==digest
def test_strict_template_binding_rejects_missing_duplicate_and_raw_fallback():
    ...
    with pytest.raises(DocumentError):
        bind_template([],{'name':'Ada'},strict=True)
    with pytest.raises(DocumentError):
        bind_template([{'key':'x','kind':'raw','locator':{}}],{'x':'v'},raw_token_fallback=False)
