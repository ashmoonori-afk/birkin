...
def test_splice_preserves_every_byte_outside_target_span():
    start,end=resolve_text_span(XML,{'paragraph_native_id':'A','run_index':1})
    out=splice_text(XML,{'paragraph_native_id':'A','run_index':1},'changed & safe')
...
def test_native_identity_survives_a_preceding_insert():
    ...
    with pytest.raises(DocumentError) as caught:
        resolve_text_span(changed,{'paragraph_index':2,'run_index':1})
