...
def test_import_is_content_addressed_deduplicated_and_source_immutable(tmp_path):
    ...
    source.write_bytes(b'office-data')
    before=hashlib.sha256(source.read_bytes()).hexdigest()
    store=ArtifactStore(tmp_path/'cas')
    a=store.import_file(source,sensitivity='confidential',acl_fingerprint='a'*64)
    b=store.import_file(source,sensitivity='internal',acl_fingerprint='b'*64)
    ...
    store.copy_out(a,out)
    assert out.read_bytes()==b'office-data'
