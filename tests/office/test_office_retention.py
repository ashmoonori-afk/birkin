...
def test_retention_skips_incomplete_job_without_disabling_office(
    tmp_path: Path,
) -> None:
    service = DocumentService(tmp_path / "office-home")
    jobs = service.home / "jobs"
    jobs.mkdir(parents=True, exist_ok=True)
    corrupt = jobs / f"{'a' * 32}.jsonl"
    _ = corrupt.write_text('{"job_id":"incomplete"', encoding="utf-8")

    removed = purge_expired_office_state(service.home)

    assert removed == {"jobs": 0, "backups": 0, "transactions": 0}
    assert corrupt.read_text(encoding="utf-8") == '{"job_id":"incomplete"'
