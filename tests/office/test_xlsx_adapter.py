...
def test_xlsx_cell_patch_preserves_formula_format_merge_chart_and_hidden_state(tmp_path):
    a=XlsxAdapter()
    before=a.part_hashes(FIX)
    out=tmp_path/'draft.xlsx'
    result=a.patch_cell(FIX,out,'A1',42)
    after=a.part_hashes(out)
    ...
    with zipfile.ZipFile(out) as z:
        ...
        assert b'<v>42</v>' in sheet and b'<f>A1*2</f>' in sheet and b'mergeCell ref="A1:B1"' in sheet
