...
- Path resolution (absolute, relative to BIRKIN_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp BIRKIN_HOME."""
    birkin_home = tmp_path / ".birkin"
    birkin_home.mkdir()
    (birkin_home / "cron").mkdir()
    (birkin_home / "cron" / "output").mkdir()
    (birkin_home / "scripts").mkdir()
    monkeypatch.setenv("BIRKIN_HOME", str(birkin_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "BIRKIN_DIR", birkin_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", birkin_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", birkin_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", birkin_home / "cron" / "output")
...
class TestCronjobToolScript:
    ...
    def test_create_with_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("BIRKIN_INTERACTIVE", "1")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.birkin/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("BIRKIN_SESSION_PLATFORM") is None
        assert os.environ.get("BIRKIN_SESSION_CHAT_ID") is None
        assert os.environ.get("BIRKIN_SESSION_CHAT_NAME") is None
