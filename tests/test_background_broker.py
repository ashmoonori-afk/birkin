...
def test_broker_rejects_work_beyond_queue_capacity(
    tmp_path: Path,
) -> None:
    background = _background_module()
    ...
    def running(context):
        running_started.set()
        assert release_running.wait(timeout=2.0)
        return "running-done"
    ...
    with background.BackgroundBroker(tmp_path, max_workers=1) as broker:
        running_job = broker.submit("running", running)
        assert running_started.wait(timeout=2.0)
        queued_job = broker.submit("queued", queued)

        with pytest.raises(RuntimeError, match="queue is full"):
            broker.submit("overflow", queued)

        release_running.set()
        assert broker.wait(running_job.id, timeout=2.0).status == "succeeded"
        assert broker.wait(queued_job.id, timeout=2.0).status == "succeeded"
...
def test_broker_close_can_skip_waiting_for_running_job(
    tmp_path: Path,
) -> None:
    background = _background_module()
    ...
    broker = background.BackgroundBroker(tmp_path, max_workers=1)
    broker.submit("blocked", blocked)
    ...
    release.set()
    broker.close(wait=True)


def test_non_waiting_close_does_not_block_interpreter_exit(
    tmp_path: Path,
) -> None:
    script = textwrap.dedent(
        ...
        import threading
        from pathlib import Path
        from birkin.background import BackgroundBroker
        ...
        broker = BackgroundBroker(Path({str(tmp_path)!r}), max_workers=1)
        broker.submit("blocked", blocked)
        assert started.wait(timeout=1.0)
        broker.submit("queued", blocked)
        broker.close(wait=False)
    ...
    assert completed.stdout.strip() == "CLOSE_RETURNED"


def test_broker_completes_when_terminal_receipt_write_fails(
    tmp_path: Path,
    ...
    with background.BackgroundBroker(tmp_path, max_workers=1) as broker:
        job = broker.submit("receipt-failure", task)
        assert started.wait(timeout=2.0)

        def fail_persist(record) -> None:
            raise OSError("receipt unavailable")

        monkeypatch.setattr(broker, "_persist", fail_persist)
        release.set()
        snapshot = broker.wait(job.id, timeout=2.0)

    assert snapshot.status == "failed"
    assert snapshot.error == "OSError: receipt unavailable"
