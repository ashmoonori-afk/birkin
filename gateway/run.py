...
# Resolve Birkin home directory (respects BIRKIN_HOME override)
from birkin_constants import get_birkin_home
from utils import atomic_yaml_write, is_truthy_value
_birkin_home = get_birkin_home()

# Load environment variables from ~/.birkin/.env first.
# User-managed env files should override stale shell exports on restart.
from dotenv import load_dotenv  # backward-compat for tests that monkeypatch this symbol
from birkin_cli.env_loader import load_birkin_dotenv
_env_path = _birkin_home / '.env'
load_birkin_dotenv(birkin_home=_birkin_home, project_env=Path(__file__).resolve().parents[1] / '.env')

# Bridge config.yaml values into the environment so os.getenv() picks them up.
# config.yaml is authoritative for terminal settings — overrides .env.
_config_path = _birkin_home / 'config.yaml'
    ...
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                os.environ["BIRKIN_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
            # Bridge agent.gateway_timeout → BIRKIN_AGENT_TIMEOUT env var.
            # Env var from .env takes precedence (already in os.environ).
            if "gateway_timeout" in _agent_cfg and "BIRKIN_AGENT_TIMEOUT" not in os.environ:
                os.environ["BIRKIN_AGENT_TIMEOUT"] = str(_agent_cfg["gateway_timeout"])
            if "gateway_timeout_warning" in _agent_cfg and "BIRKIN_AGENT_TIMEOUT_WARNING" not in os.environ:
                os.environ["BIRKIN_AGENT_TIMEOUT_WARNING"] = str(_agent_cfg["gateway_timeout_warning"])
            if "gateway_notify_interval" in _agent_cfg and "BIRKIN_AGENT_NOTIFY_INTERVAL" not in os.environ:
                os.environ["BIRKIN_AGENT_NOTIFY_INTERVAL"] = str(_agent_cfg["gateway_notify_interval"])
            if "restart_drain_timeout" in _agent_cfg and "BIRKIN_RESTART_DRAIN_TIMEOUT" not in os.environ:
                os.environ["BIRKIN_RESTART_DRAIN_TIMEOUT"] = str(_agent_cfg["restart_drain_timeout"])
        _display_cfg = _cfg.get("display", {})
        if _display_cfg and isinstance(_display_cfg, dict):
            if "busy_input_mode" in _display_cfg and "BIRKIN_GATEWAY_BUSY_INPUT_MODE" not in os.environ:
                os.environ["BIRKIN_GATEWAY_BUSY_INPUT_MODE"] = str(_display_cfg["busy_input_mode"])
        # Timezone: bridge config.yaml → BIRKIN_TIMEZONE env var.
        # BIRKIN_TIMEZONE from .env takes precedence (already in os.environ).
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str) and "BIRKIN_TIMEZONE" not in os.environ:
            os.environ["BIRKIN_TIMEZONE"] = _tz_cfg.strip()
        # Security settings
        _security_cfg = _cfg.get("security", {})
        if isinstance(_security_cfg, dict):
            _redact = _security_cfg.get("redact_secrets")
            if _redact is not None:
                os.environ["BIRKIN_REDACT_SECRETS"] = str(_redact).lower()
    except Exception:
        pass  # Non-fatal; gateway can still run with .env values

# Apply IPv4 preference if configured (before any HTTP clients are created).
try:
    from birkin_constants import apply_ipv4_preference
...
os.environ["BIRKIN_QUIET"] = "1"

# Enable interactive exec approval for dangerous commands on messaging platforms
os.environ["BIRKIN_EXEC_ASK"] = "1"
...
def _expand_whatsapp_auth_aliases(identifier: str) -> set:
    ...
    session_dir = _birkin_home / "whatsapp" / "session"
    resolved = set()
    queue = [normalized]
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    try:
        runtime = resolve_runtime_provider(
            requested=os.getenv("BIRKIN_INFERENCE_PROVIDER"),
        )
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if name == normalized and name in disabled:
                    return (
                        ...
                        f"Enable it with: `birkin skills config`"
                    )

        # Check optional skills (shipped with repo but not installed)
        from birkin_constants import get_optional_skills_dir
            ...
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                if name == normalized:
                    ...
                    return (
                        ...
                        f"Install it with: `birkin skills install {install_path}`"
                    )
    except Exception:
        pass
    return None
...
def _load_gateway_config() -> dict:
    """Load and parse ~/.birkin/config.yaml, returning {} on any error."""
    try:
        config_path = _birkin_home / 'config.yaml'
        if config_path.exists():
            import yaml
            with open(config_path, 'r', encoding='utf-8') as f:
                return yaml.safe_load(f) or {}
    except Exception:
        logger.debug("Could not load gateway config from %s", _birkin_home / 'config.yaml')
    return {}
...
def _resolve_birkin_bin() -> Optional[list[str]]:
    """Resolve the Birkin update command as argv parts.

    Tries in order:
    1. ``shutil.which("birkin")`` — standard PATH lookup
    2. ``sys.executable -m birkin_cli.main`` — fallback when Birkin is running
       from a venv/module invocation and the ``birkin`` shim is not on PATH
    ...
    try:
        ...
        if importlib.util.find_spec("birkin_cli") is not None:
            return [sys.executable, "-m", "birkin_cli.main"]
    except Exception:
        pass
...
class GatewayRunner:
    ...
    def _has_setup_skill(self) -> bool:
        """Check if the birkin-agent-setup skill is installed."""
        try:
            from tools.skill_manager_tool import _find_skill
            return _find_skill("birkin-agent-setup") is not None
        except Exception:
            return False

    # -- Voice mode persistence ------------------------------------------

    _VOICE_MODE_PATH = _birkin_home / "gateway_voice_mode.json"
    ...
    def _resolve_session_agent_runtime(
        ...
        *,
        ...
        # (e.g. user ran `birkin auth add openai-codex` without `birkin model`),
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks BIRKIN_PREFILL_MESSAGES_FILE env var first, then falls back to
        the prefill_messages_file key in ~/.birkin/config.yaml.
        Relative paths are resolved from ~/.birkin/.
        """
        import json as _json
        file_path = os.getenv("BIRKIN_PREFILL_MESSAGES_FILE", "")
        if not file_path:
            try:
                import yaml as _y
                cfg_path = _birkin_home / "config.yaml"
                if cfg_path.exists():
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks BIRKIN_EPHEMERAL_SYSTEM_PROMPT env var first, then falls back to
        agent.system_prompt in ~/.birkin/config.yaml.
        """
        prompt = os.getenv("BIRKIN_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
        try:
            import yaml as _y
    ...
    def _load_busy_input_mode() -> str:
        ...
        mode = os.getenv("BIRKIN_GATEWAY_BUSY_INPUT_MODE", "").strip().lower()
        if not mode:
            try:
                import yaml as _y
                cfg_path = _birkin_home / "config.yaml"
    ...
    def _load_restart_drain_timeout() -> float:
        ...
        raw = os.getenv("BIRKIN_RESTART_DRAIN_TIMEOUT", "").strip()
        if not raw:
            try:
                import yaml as _y
                cfg_path = _birkin_home / "config.yaml"
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = os.getenv("BIRKIN_BACKGROUND_NOTIFICATIONS", "")
        if not mode:
            try:
                import yaml as _y
                cfg_path = _birkin_home / "config.yaml"
    ...
    async def _launch_detached_restart_command(self) -> None:
        ...
        birkin_cmd = _resolve_birkin_bin()
        if not birkin_cmd:
            logger.error("Could not locate birkin binary for detached /restart")
            return

        current_pid = os.getpid()
        cmd = " ".join(shlex.quote(part) for part in birkin_cmd)
        shell_cmd = (
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting Birkin Gateway...")
        ...
        if not _any_allowlist and not _allow_all:
            logger.warning(
                ...
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.birkin/.env to allow open access, "
        ...
        # This prevents unwanted auto-resets after `birkin update`,
        # `birkin gateway restart`, or `/restart`.
        _clean_marker = _birkin_home / ".clean_shutdown"
        ...
        if not notified and any(
            path.exists()
            for path in (
                _birkin_home / ".update_pending.json",
                _birkin_home / ".update_pending.claimed.json",
            )
        ):
    ...
    async def stop(
        ...
        *,
        ...
        async def _stop_impl() -> None:
            ...
            try:
                (_birkin_home / ".clean_shutdown").touch()
            except Exception:
                pass
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            if not check_slack_requirements():
                logger.warning("Slack: slack-bolt not installed. Run: pip install 'birkin-agent[slack]'")
                return None
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
                ...
                if code:
                    ...
                    if adapter:
                        await adapter.send(
                            ...
                            f"`birkin pairing approve {platform_name} {code}`"
                        )
                else:
                    adapter = self.adapters.get(source.platform)
                    if adapter:
        ...
        if _update_prompts.get(_quick_key):
            ...
            if cmd in ("approve", "yes"):
                ...
                response_text = raw
            if response_text:
                response_path = _birkin_home / ".update_response"
                try:
        ...
        _raw_stale_timeout = float(os.getenv("BIRKIN_AGENT_TIMEOUT", 1800))
        ...
        if _quick_key in self._running_agents:
            ...
            from birkin_cli.commands import resolve_command as _resolve_cmd_inner
        ...
        # in birkin_cli/commands.py — no hardcoded set to maintain here.
        from birkin_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command as _resolve_cmd
        ...
        if command:
            try:
                ...
                # hyphens. See birkin_cli/commands.py:_build_telegram_menu.
                    ...
                    if _stt_adapter:
                        try:
                            _stt_msg = (
                                ...
                                "(`pip install faster-whisper` in the Birkin venv) "
                                "and set `stt.enabled: true` in config.yaml, "
                                "then /restart the gateway."
                            )
                            if self._has_setup_skill():
                                _stt_msg += "\n\nFor full setup instructions, type: `/skill birkin-agent-setup`"
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        if history and len(history) >= 4:
            ...
            _hyg_data = {}
            try:
                _hyg_cfg_path = _birkin_home / "config.yaml"
                ...
                if _hyg_config_context_length is None and _hyg_base_url:
                    try:
                        try:
                            from birkin_cli.config import get_compatible_custom_providers as _gw_gcp
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where Birkin delivers cron job results "
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        # Detect profile name from BIRKIN_HOME path
        # Profile paths look like: ~/.birkin/profiles/<name>
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            (_birkin_home / ".restart_notify.json").write_text(
        ...
        return "♻ Restarting gateway. If you aren't notified within 60 seconds, restart from the console with `birkin gateway restart`."

    async def _handle_help_command(self, event: MessageEvent) -> str:
        """Handle /help command - list available commands."""
        from birkin_cli.commands import gateway_help_lines
        lines = [
            "📖 **Birkin Commands**\n",
            *gateway_help_lines(),
        ]
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        try:
            if config_path.exists():
                ...
                try:
                    from birkin_cli.config import get_compatible_custom_providers
        ...
        if persist_global:
            try:
                ...
                from birkin_cli.config import save_config
                save_config(cfg)
            except Exception as e:
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        lines.append("Setup: `birkin setup`")
        return "\n".join(lines)
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.birkin/config.yaml`"
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall Birkin with the messaging extra, e.g. "
                    "`pip install birkin-agent[messaging]`."
                )
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _birkin_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _run_background_task(
        ...
        try:
            ...
            max_iterations = int(os.getenv("BIRKIN_MAX_ITERATIONS", "90"))
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        def _collect_and_upload():
            ...
            lines.append("\nShare these links with the Birkin team for support.")
            return "\n".join(lines)

        return await loop.run_in_executor(None, _collect_and_upload)

    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Birkin to the latest version.

        Spawns ``birkin update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``birkin update`` may trigger. Marker
        ...
        import subprocess
        from datetime import datetime
        from birkin_cli.config import is_managed, format_managed_message

        # Block non-messaging platforms (API server, webhooks, ACP)
        platform = event.source.platform
        if platform not in self._UPDATE_ALLOWED_PLATFORMS:
            return "✗ /update is only available from messaging platforms. Run `birkin update` from the terminal."

        if is_managed():
            return f"✗ {format_managed_message('update Birkin')}"
        ...
        if not birkin_cmd:
            return (
                "✗ Could not locate the `birkin` command. "
                "Birkin is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `birkin update` manually in your terminal."
            )

        pending_path = _birkin_home / ".update_pending.json"
        output_path = _birkin_home / ".update_output.txt"
        exit_code_path = _birkin_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `birkin update --gateway` detached so it survives gateway restart.
        ...
        birkin_cmd_str = " ".join(shlex.quote(part) for part in birkin_cmd)
        update_cmd = (
            f"PYTHONUNBUFFERED=1 {birkin_cmd_str} update --gateway"
        ...
        self._schedule_update_notification_watch()
        return "⚕ Starting Birkin update… I'll stream progress here."
    ...
    async def _watch_update_progress(
        ...
        """Watch ``birkin update --gateway``, streaming output + forwarding prompts.
        ...
        pending_path = _birkin_home / ".update_pending.json"
        claimed_path = _birkin_home / ".update_pending.claimed.json"
        output_path = _birkin_home / ".update_output.txt"
        exit_code_path = _birkin_home / ".update_exit_code"
        prompt_path = _birkin_home / ".update_prompt.json"
        ...
        while loop.time() < deadline:
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    if exit_code == 0:
                        await adapter.send(chat_id, "✅ Birkin update finished.")
                    else:
                        await adapter.send(chat_id, "❌ Birkin update failed (exit code {}).".format(exit_code))
                ...
                (_birkin_home / ".update_response").unlink(missing_ok=True)
        ...
        if not exit_code_path.exists():
            ...
            await _flush_buffer()
            try:
                await adapter.send(chat_id, "❌ Birkin update timed out after 30 minutes.")
            except Exception:
                pass
            for p in (pending_path, claimed_path, output_path,
                      exit_code_path, prompt_path):
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    if exit_code == 0:
                        msg = f"✅ Birkin update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"❌ Birkin update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "✅ Birkin update finished successfully."
                    else:
                        msg = "❌ Birkin update failed. Check the gateway logs or run `birkin update` manually for details."
    ...
    async def _send_restart_notification(self) -> None:
        ...
        notify_path = _birkin_home / ".restart_notify.json"
    ...
    async def _enrich_message_with_transcription(
        ...
        if not getattr(self.config, "stt_enabled", True):
            ...
            if self._has_setup_skill():
                disabled_note += (
                    " You have a skill called birkin-agent-setup that can help "
                    "users configure Birkin features including voice, tools, and more."
                )
            disabled_note += "]"
            if user_text:
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if (
                        ...
                        if self._has_setup_skill():
                            _no_stt_note += (
                                " You have a skill called birkin-agent-setup "
                                "that can help users configure Birkin features "
    ...
    async def _run_agent(
        ...
        progress_mode = (
            _resolved_tp
            or os.getenv("BIRKIN_TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        def run_sync():
            ...
            os.environ["BIRKIN_SESSION_KEY"] = session_key or ""
        ...
        # BIRKIN_AGENT_NOTIFY_INTERVAL env var.  Default 600s (10 min).
        # 0 = disable notifications.
        _NOTIFY_INTERVAL_RAW = float(os.getenv("BIRKIN_AGENT_NOTIFY_INTERVAL", 600))
        ...
        try:
            ...
            # BIRKIN_AGENT_TIMEOUT env var (env var takes precedence).
            # Default 1800s (30 min inactivity).  0 = unlimited.
            _agent_timeout_raw = float(os.getenv("BIRKIN_AGENT_TIMEOUT", 1800))
            _agent_timeout = _agent_timeout_raw if _agent_timeout_raw > 0 else None
            _agent_warning_raw = float(os.getenv("BIRKIN_AGENT_TIMEOUT_WARNING", 900))
            ...
            if pending and pending.strip().startswith("/"):
                ...
                if _pending_cmd_word:
                    try:
                        from birkin_cli.commands import resolve_command as _rc_pending
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, loop=None, interval: int = 60):
    ...
    needing a separate `birkin cron daemon` or system cron entry.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Prevent two gateways from running under the same BIRKIN_HOME.
    # The PID file is scoped to BIRKIN_HOME, so future multi-profile
    # setups (each profile using a distinct BIRKIN_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            birkin_home = str(get_birkin_home())
            logger.error(
                "Another gateway instance is already running (PID %d, BIRKIN_HOME=%s). "
                "Use 'birkin gateway restart' to replace it, or 'birkin gateway stop' first.",
                existing_pid, birkin_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'birkin gateway restart' to replace it,\n"
                f"   or 'birkin gateway stop' to kill it first.\n"
                f"   Or use 'birkin gateway run --replace' to auto-replace.\n"
            )
            return False
    ...
    setup_logging(birkin_home=_birkin_home, mode="gateway")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Birkin Gateway - Multi-platform messaging")
