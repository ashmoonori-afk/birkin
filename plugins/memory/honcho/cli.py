...
Handles: birkin honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "birkin" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Birkin profiles and creates host blocks for any that don't
    have one yet. Inherits settings from the default host block.
    """
    try:
        from birkin_cli.profiles import list_profiles
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'birkin honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'birkin honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `birkin update` -- no output, no exceptions.
    """
    try:
        from birkin_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Birkin profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $BIRKIN_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_birkin_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Birkin persistent cross-session memory.")
    ...
    birkin_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'birkin honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = birkin_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        birkin_host["peerName"] = new_peer

    current_ai = birkin_host.get("aiPeer") or cfg.get("aiPeer", "birkin")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        birkin_host["aiPeer"] = new_ai

    current_workspace = birkin_host.get("workspace") or cfg.get("workspace", "birkin")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        birkin_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = birkin_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in ("unified", "directional"):
        birkin_host["observationMode"] = new_obs
    else:
        birkin_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(birkin_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        birkin_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        birkin_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # --- 6. Recall mode ---
    _raw_recall = birkin_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        birkin_host["recallMode"] = new_recall

    # --- 7. Session strategy ---
    current_strat = birkin_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-directory")
    print("\n  Session strategy:")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        birkin_host["sessionStrategy"] = new_strat

    birkin_host["enabled"] = True
    birkin_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        birkin_config = load_config()
        birkin_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(birkin_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: birkin config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    birkin honcho status     -- show full config")
    print("    birkin honcho mode       -- change recall/observation mode")
    print("    birkin honcho tokens     -- tune context and dialectic budgets")
    print("    birkin honcho peer       -- update peer names")
    print("    birkin honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Birkin profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: birkin honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'birkin honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: birkin honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        birkin = hosts.get(_host_key(), {})
        user = birkin.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = birkin.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = birkin.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = birkin.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Birkin' identity in Honcho. Seed with 'birkin honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: birkin honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = birkin.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = birkin.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = birkin.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Birkin asks Honcho's AI peer a question")
        ...
        print("\n  Set with: birkin honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'birkin honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'birkin honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    birkin honcho identity --show        — show both peer representations")
        print("    birkin honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Birkin + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Birkin\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Birkin persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  birkin honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'birkin honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'birkin honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  birkin honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'birkin'")
        print("  with Honcho configured and no prior session history.")
        print("  (Birkin calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    birkin honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'birkin honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Birkin, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'birkin honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    birkin honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  birkin honcho identity <path/to/SOUL.md>")
    ...
    print("    Birkin:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Birkin:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Birkin:   Honcho observes every message and updates representations")
    ...
    print("    Birkin:   per-session by default — each run gets its own session")
    print("              Map a custom name:  birkin honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. birkin honcho setup              — configure API key (required)")
        print("  2. birkin honcho migrate            — re-run this walkthrough")
    else:
        print("  1. birkin honcho status             — verify Honcho connection")
        print("  2. birkin                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. birkin honcho identity --show    — verify AI peer representation")
        print("  4. birkin honcho tokens             — tune context and dialectic budgets")
        print("  5. birkin honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'birkin memory setup'...\n")
        from birkin_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``birkin honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``birkin honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to birkin memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Birkin Honcho",
    )
