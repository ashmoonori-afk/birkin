...
def _load_supermemory_config(birkin_home: str) -> dict:
    config = _default_config()
    config_path = Path(birkin_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, birkin_home: str) -> None:
    config_path = Path(birkin_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `birkin memory setup`.
        # All other options are documented for $BIRKIN_HOME/supermemory.json
    ...
    def save_config(self, values, birkin_home):
        ...
        _save_supermemory_config(sanitized, birkin_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from birkin_constants import get_birkin_home
        self._birkin_home = kwargs.get("birkin_home") or str(get_birkin_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._birkin_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "birkin", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "birkin_memory", "target": target, "type": "explicit_memory"},
