...
Config file: `$BIRKIN_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `birkin` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `birkin-{identity}` → `birkin-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Birkin profile:

```json
{
  "container_tag": "birkin-{identity}"
}
```

For a profile named `coder`, this resolves to `birkin-coder`. The default profile resolves to `birkin-default`. Without `{identity}`, all profiles share the same container.
