...
if tinker_atropos_dir.exists():
    ...
    os.environ['BIRKIN_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to birkin-agent directory if submodule not found
...
from birkin_constants import get_birkin_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_birkin_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.birkin/config.yaml if not provided)
    ...
    """
    # Load config from ~/.birkin/config.yaml
    config = load_birkin_config()
