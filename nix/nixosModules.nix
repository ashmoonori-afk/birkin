# nix/nixosModules.nix — NixOS module for birkin-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: birkin runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $BIRKIN_HOME/.env
# and read by birkin at startup — no container recreation needed for env changes.
#
# Tool resolution: the birkin wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.birkin-agent;
    birkin-agent = inputs.self.packages.${pkgs.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-birkin-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "birkin-config-attrs";
      description = "Birkin YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "birkin-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "birkin-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'BIRKIN_DOC_EOF'\n${value}\nBIRKIN_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "birkin-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/birkin";
    ...
    # birkin user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "birkin-container-entrypoint" ''
      set -eu

      BIRKIN_UID="''${BIRKIN_UID:?BIRKIN_UID must be set}"
      BIRKIN_GID="''${BIRKIN_GID:?BIRKIN_GID must be set}"

      # ── Group: ensure a group with GID=$BIRKIN_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$BIRKIN_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="birkin"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$BIRKIN_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$BIRKIN_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$BIRKIN_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$BIRKIN_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="birkin"
        TARGET_HOME="/home/birkin"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$BIRKIN_UID" -g "$BIRKIN_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$BIRKIN_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$BIRKIN_UID:$BIRKIN_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure BIRKIN_HOME is owned by the target user
      if [ -n "''${BIRKIN_HOME:-}" ] && [ -d "$BIRKIN_HOME" ]; then
        chown -R "$BIRKIN_UID:$BIRKIN_GID" "$BIRKIN_HOME"
      fi
      ...
      if [ ! -f /var/lib/birkin-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        echo "First boot: provisioning agent tools..."
        apt-get update -qq
        apt-get install -y -qq sudo nodejs npm curl
        touch /var/lib/birkin-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/birkin ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/birkin
        chmod 0440 /etc/sudoers.d/birkin
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$BIRKIN_UID" --regid="$BIRKIN_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $BIRKIN_HOME/.env.
    ...
    # Default: /var/lib/birkin/workspace → /data/workspace.
  ...
  in {
    options.services.birkin-agent = with lib; {
      enable = mkEnableOption "Birkin gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = birkin-agent;
        description = "The birkin-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/birkin";
        description = "State directory. Contains .birkin/ subdir (BIRKIN_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Birkin config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $BIRKIN_HOME/.env at activation time.
          Birkin reads this file on every startup via load_birkin_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $BIRKIN_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $BIRKIN_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `birkin gateway`.";
      };
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the birkin CLI to environment.systemPackages and export
          BIRKIN_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.birkin symlink to the service
            stateDir. These users are automatically added to the birkin group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.birkin-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the birkin CLI to system PATH and export BIRKIN_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.birkin/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ cfg.package ];
        environment.variables.BIRKIN_HOME = "${cfg.stateDir}/.birkin";
      })
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.birkin-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed birkin
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure birkin is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.birkin        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.birkin/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.birkin/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.birkin/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.birkin/memories 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."birkin-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.birkin
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.birkin ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.birkin ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.birkin -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories; do
            mkdir -p "${cfg.stateDir}/.birkin/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.birkin/$_subdir"
            chmod 2770 "${cfg.stateDir}/.birkin/$_subdir"
            find "${cfg.stateDir}/.birkin/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.birkin/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.birkin/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.birkin/config.yaml
            chmod 0640 ${cfg.stateDir}/.birkin/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.birkin/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.birkin/.managed
          chmod 0644 ${cfg.stateDir}/.birkin/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.birkin/.container-mode <<'BIRKIN_CONTAINER_MODE_EOF'
# Written by NixOS activation script. Do not edit manually.
backend=${cfg.container.backend}
container_name=${containerName}
exec_user=${cfg.user}
birkin_bin=${containerDataDir}/current-package/bin/birkin
BIRKIN_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.birkin/.container-mode
            chmod 0644 ${cfg.stateDir}/.birkin/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.birkin/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.birkin";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.birkin" ]; then
                  rm -f "${symlinkPath}"
                  echo "birkin-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.birkin -> stateDir/.birkin for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.birkin";
                target = "${cfg.stateDir}/.birkin";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "birkin-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.birkin/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.birkin/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.birkin/auth.json
              fi
            ''}
          ''}
          ...
          # Birkin reads $BIRKIN_HOME/.env at startup via load_birkin_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.birkin/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'BIRKIN_NIX_ENV_EOF'
${envFileContent}
BIRKIN_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.birkin-agent = {
          description = "Birkin Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            BIRKIN_HOME = "${cfg.stateDir}/.birkin";
            BIRKIN_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $BIRKIN_HOME/.env by the activation script. load_birkin_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${cfg.package}/bin/birkin"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the birkin group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.birkin-agent = {
          description = "Birkin Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              BIRKIN_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              BIRKIN_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env BIRKIN_UID="$BIRKIN_UID" \
                --env BIRKIN_GID="$BIRKIN_GID" \
                --env BIRKIN_HOME=${containerDataDir}/.birkin \
                --env BIRKIN_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/birkin gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
