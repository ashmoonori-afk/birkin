...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      birkin-agent = inputs.self.packages.${system}.default;
      ...
      configKeys = pkgs.runCommand "birkin-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${birkinVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        package-contents = pkgs.runCommand "birkin-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${birkin-agent}/bin/birkin || (echo "FAIL: birkin binary missing"; exit 1)
          test -x ${birkin-agent}/bin/birkin-agent || (echo "FAIL: birkin-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${birkin-agent}/bin/birkin version 2>&1 | grep -qi "birkin" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "birkin-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in birkin birkin-agent birkin-acp; do
            test -x ${birkin-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "birkin-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking birkin --help ==="
          ${birkin-agent}/bin/birkin --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${birkin-agent}/bin/birkin --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "birkin-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${birkin-agent}/share/birkin-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${birkin-agent}/share/birkin-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "BIRKIN_BUNDLED_SKILLS" ${birkin-agent}/bin/birkin || \
            (echo "FAIL: BIRKIN_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: BIRKIN_BUNDLED_SKILLS set in wrapper"
        ...
        # Verify BIRKIN_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "birkin-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(BIRKIN_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking BIRKIN_MANAGED guards ==="
          check_blocked "config set" ${birkin-agent}/bin/birkin config set model foo
          check_blocked "config edit" ${birkin-agent}/bin/birkin config edit
        ...
        in pkgs.runCommand "birkin-config-roundtrip" {
          ...
          merge_and_load() {
            local birkin_home="$1"
            export BIRKIN_HOME="$birkin_home"
            ${configMergeScript} ${nixSettings} "$birkin_home/config.yaml"
            ${birkinVenv}/bin/python3 -c '
import json, sys
