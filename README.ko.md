...
- **run_shell 승인 게이트.** `rm -rf`·`curl | sh`·force-push 같은 건 즉시 거부하거나,
  ...
  안전벨트 — 이고, 영구 허용목록은 복합 명령엔 절대 매칭되지 않습니다. Windows
  셸 작업은 `cmd.exe`로 실행되고(PowerShell은 명시적으로 요청한 경우만 사용),
  쓰기 가능한 `TEMP`/`TMP`를 받아 장시간 실행 중인 게이트웨이나 데몬에서도
  Bun·npm 같은 도구가 정상 동작합니다.
