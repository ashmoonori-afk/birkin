...
Python 3.10 이상이 필요합니다. 현재 저장소의 소스 버전은 Birkin `0.4.423`입니다. 이 표기는 같은 버전이 사용자 PC에 설치됐거나 서명된 공개 앱으로 배포됐다는 뜻이 아닙니다.
