"""Real Windows UIA acceptance using only a Birkin-owned WinForms fixture."""
...
def _ready_pid(process: subprocess.Popen[str]) -> int:
    ...
    try:
        ...
        raise TimeoutError("WinForms fixture did not report readiness.")
    prefix, raw_pid = line.strip().split(" ", 1)
    if prefix != "READY":
...
def run(fixture: Path, evidence_root: Path) -> int:
    os.environ["BIRKIN_HOME"] = str(evidence_root / "windows" / "home")
    ...
    procreg.register(
        ...
        purpose="windows-computer-use-qa-fixture",
        deadline=datetime.now(timezone.utc) + timedelta(minutes=3),
    )
    ledger = evidence_root / "windows" / "ledger.json"
    try:
        pid = _ready_pid(process)
        backend = WindowsBackend()
        service = ComputerUseService(
            backend=backend,
            artifact_store=ArtifactStore(evidence_root / "windows" / "artifacts"),
            session_id="windows-acceptance",
        ...
        typed = service.execute(
            {
                ...
                "action": "type",
                "session_id": service.session_id,
                "action_id": "windows-type",
                "idempotency_key": "windows-type-1",
                "target": {
                    "app_ref": ax["app_ref"],
        ...
        background_request = {
            ...
            "action": "double_click",
            "session_id": service.session_id,
            "action_id": "windows-background-double-click",
            "idempotency_key": "windows-background-double-click",
            "target": target,
            "delivery": "background",
            "predicted_effect": {
                "property": "name",
        ...
        foreground = service.execute(
            {
                **background_request,
                "action_id": "windows-foreground-double-click",
                "idempotency_key": "windows-foreground-double-click",
                "delivery": "foreground",
